 ...
 * ResultSchema：结构化教学结果类型 - ClaimVerifier：基于 claim
 * 的防编造校验（coordinate/numeric/pv/motif/joseki/life_death/sente_gote/ownership/student_profile） -
...
public final class ClaimVerifier {
  ...
  public static class ProfileUpdates {
    public List<String> errorTypes = new ArrayList<>();
    public List<String> patterns = new ArrayList<>();
    public List<String> trainingFocus = new ArrayList<>();
  }
  ...
  public static class GroundedTeachingSection {
    public String id, section, markdown;
    public List<String> claimIds = new ArrayList<>();
  }

  public static class GroundedTeachingOutput {
    public int schemaVersion = 1;
    public String headline, summary, confidence, finalMarkdown;
  ...
  static List<String> allowedMoves(TeachingEvidenceBuilder.TeachingEvidence ev) {
    ...
    for (TeachingEvidenceBuilder.RecognizedMotifView motif : ev.recognizedMotifs) {
      if (motif.relatedMoves != null)
        for (String m : motif.relatedMoves) if (m != null) values.add(m.toUpperCase());
      if (motif.expectedNextMoves != null)
        for (Object o : motif.expectedNextMoves) values.add(o.toString().toUpperCase());
    }
  ...
  static List<Double> extractPercentages(String text) {
    ...
    while (mt.find()) {
      try {
        out.add(Double.parseDouble(mt.group(1)));
  ...
  static boolean hasSupportedJoseki(TeachingEvidenceBuilder.TeachingEvidence ev) {
    for (TeachingEvidenceBuilder.RecognizedMotifView m : ev.recognizedMotifs)
      if (m.motifType != null
          ...
          && ("strong".equals(m.confidence) || "medium".equals(m.confidence))) return true;
    return false;
  }
  ...
  static boolean hasTacticalSupport(TeachingEvidenceBuilder.TeachingEvidence ev) {
    for (TeachingEvidenceBuilder.RecognizedMotifView m : ev.recognizedMotifs) {
      String combined =
          (m.motifType != null ? m.motifType : "") + " " + (m.title != null ? m.title : "");
      if (combined.matches(
              ".*(life|death|tesuji|cut|connect|ladder|net|snapback|throw|eye|semeai|ko).*")
          && ("strong".equals(m.confidence) || "medium".equals(m.confidence))) return true;
    }
    return false;
  }

  static boolean near(double value, double target, double tol) {
    return Math.abs(value - target) <= tol;
  }

  static void verifyNumericText(
      String text,
      TeachingEvidenceBuilder.TeachingEvidence ev,
      List<String> warnings,
      List<String> violations) {
    for (double percent : extractPercentages(text)) {
      if (percent > 100) {
        violations.add("Impossible percentage " + percent + "%.");
        continue;
      }
      ...
      for (TeachingEvidenceBuilder.TeachingEvidenceCandidate c : ev.bestCandidates)
        if (c.winrate != null) known.add(round(c.winrate, 1));
      if (!known.stream().anyMatch(t -> near(percent, t, 0.6)))
        warnings.add(
            "Percentage " + percent + "% is not close to any provided winrate/loss evidence.");
    }
    List<Double> scoreMentions = new ArrayList<>();
    if (text != null) {
      Matcher mt =
          Pattern.compile("(?:目差|亏|领先|落后|score|points?).{0,8}?(-?\\d+(?:\\.\\d+)?)").matcher(text);
      while (mt.find()) {
        try {
          scoreMentions.add(Double.parseDouble(mt.group(1)));
        } catch (Exception ignore) {
        }
      }
    }
    ...
    if (ev.afterActual.blackScoreLead != null)
      knownScores.add(round(ev.afterActual.blackScoreLead, 1));
    ...
    for (double sc : scoreMentions)
      if (!knownScores.stream().anyMatch(t -> near(sc, t, 0.8)))
        warnings.add("Score/point value " + sc + " is not close to provided score evidence.");
  }

  public static ClaimVerificationResult verifyGroundedClaims(
      List<GroundedTeachingClaim> claims, TeachingEvidenceBuilder.TeachingEvidence ev) {
    ...
    for (GroundedTeachingClaim claim : claims) {
      if (claim.text == null || claim.text.trim().isEmpty()) {
        warnings.add("Claim " + claim.id + " is empty.");
        continue;
      }
      if (claim.evidenceRefs == null || claim.evidenceRefs.isEmpty())
        warnings.add("Claim " + claim.id + " has no evidenceRefs.");
      for (String coord : extractCoordinates(claim.text))
        if (!allowedSet.contains(coord))
          violations.add("Claim " + claim.id + " mentions unsupported coordinate " + coord + ".");
      if (claim.type == GroundedClaimType.numeric)
        verifyNumericText(claim.text, ev, warnings, violations);
      if (claim.type == GroundedClaimType.joseki && !hasSupportedJoseki(ev))
        violations.add(
            "Claim " + claim.id + " names joseki without medium/strong joseki motif evidence.");
      if ((claim.type == GroundedClaimType.life_death || claim.type == GroundedClaimType.sente_gote)
          && "high".equals(claim.confidence)
          && !hasTacticalSupport(ev))
        warnings.add(
            "Claim "
                + claim.id
                + " is high-confidence tactical claim without explicit tactical motif support.");
      if (ev.loss.confidence != TeachingEvidenceBuilder.TeachingConfidence.high
          && claim.text.matches(".*(唯一|必然|必杀|净杀|必活|绝对|certain|only\\s+move|forced).*"))
        violations.add(
            "Claim "
                + claim.id
                + " is too absolute for "
  ...
  public static List<GroundedTeachingClaim> claimsFromMarkdown(String markdown) {
    ...
    for (String item : parts) {
      ...
      c.confidence = "medium";
      if (Pattern.compile("\\b[A-HJ-T](?:1?\\d|2[0-5])\\b").matcher(text).find())
        c.type = GroundedClaimType.coordinate;
      else if (Pattern.compile("\\d+(?:\\.\\d+)?\\s*%|目差|score|points?")
          .matcher(text.toLowerCase())
          .find()) c.type = GroundedClaimType.numeric;
      else if (text.matches(".*(定式|joseki|定石|정석).*")) c.type = GroundedClaimType.joseki;
      else if (text.matches(".*(死活|做活|杀棋|眼|气|libert|life|death).*"))
        c.type = GroundedClaimType.life_death;
  ...
  public static ClaimVerificationResult verifyTeacherClaimsFromMarkdown(
      String markdown, TeachingEvidenceBuilder.TeachingEvidence ev) {
  ...
  public static String buildClaimVerificationNote(ClaimVerificationResult r) {
    ...
    issues.addAll(r.violations);
    issues.addAll(r.warnings);
    if (issues.size() > 4) issues = issues.subList(0, 4);
    if (issues.isEmpty())
      return "> Claim verifier: checked "
          + r.checkedClaims
          + " claims; no unsupported coordinates, impossible percentages, or over-absolute claims found.";
    return "> Claim verifier: checked "
        + r.checkedClaims
        + " claims; notes: "
        + String.join("；", issues);
  ...
  public static String normalizeJsonCandidate(String text) {
    ...
    Pattern marker =
        Pattern.compile(
            GOAGENT_GROUNDING_JSON_MARKER + "\\s*:?\\s*(\\{[\\s\\S]*?\\})\\s*(?:$|\\n)",
            Pattern.CASE_INSENSITIVE);
    Matcher m = marker.matcher(text);
    if (m.find()) return m.group(1).trim();
    Matcher fenced =
        Pattern.compile(
                "```(?:json|goagent-grounding-json|goagent_grounding_json)\\s*([\\s\\S]*?)```",
                Pattern.CASE_INSENSITIVE)
            .matcher(text);
  ...
  public static class StructuredTeachingValidation {
    public boolean ok;
    public List<String> warnings = new ArrayList<>(), violations = new ArrayList<>();
    public GroundedTeachingOutput result;
  }
  ...
  public static StructuredTeachingValidation validateGroundedTeachingResult(Object value) {
    ...
    if (!(value instanceof Map)) {
      v.violations.add("Structured teaching result must be an object.");
      return v;
    }
    Map<String, Object> m = (Map<String, Object>) value;
    if (!Integer.valueOf(1).equals(m.get("schemaVersion")))
      v.violations.add("schemaVersion must be 1.");
    for (String key : new String[] {"headline", "summary", "finalMarkdown"}) {
      Object o = m.get(key);
      if (!(o instanceof String) || ((String) o).trim().isEmpty())
        v.violations.add(key + " must be a non-empty string.");
    }
    if (!java.util.Arrays.asList("high", "medium", "low")
        .contains(String.valueOf(m.get("confidence"))))
      v.violations.add("confidence must be high, medium, or low.");
    ...
    List<String> drills = asStringArray(m.get("drills"));
    if (drills == null) v.violations.add("drills must be an array of strings.");
    List<String> fqs = asStringArray(m.get("followupQuestions"));
    if (fqs == null) v.violations.add("followupQuestions must be an array of strings.");
    ...
    if (m.get("claims") instanceof List) {
      ...
      for (Object raw : (List<?>) m.get("claims")) {
        if (!(raw instanceof Map)) {
          v.violations.add("claims[" + i + "] must be an object.");
          i++;
          continue;
        }
        ...
        if (!id.isEmpty() && claimIds.contains(id))
          v.violations.add("Duplicate claim id " + id + ".");
        ...
        if (!java.util.Arrays.asList(
                ...
                "ownership",
                "student_profile")
            .contains(type)) v.violations.add("claims[" + i + "].type is invalid.");
        if (ctext.isEmpty()) v.violations.add("claims[" + i + "].text is required.");
        if (refs == null || refs.isEmpty())
          v.violations.add("claims[" + i + "].evidenceRefs must be non-empty.");
        if (!java.util.Arrays.asList("high", "medium", "low").contains(conf))
          v.violations.add("claims[" + i + "].confidence is invalid.");
        if (!id.isEmpty()
            && !type.isEmpty()
            && !ctext.isEmpty()
            && refs != null
            && !refs.isEmpty()
            && java.util.Arrays.asList("high", "medium", "low").contains(conf)) {
          GroundedTeachingClaim gtc = new GroundedTeachingClaim();
          gtc.id = id;
          gtc.type = GroundedClaimType.valueOf(type);
          gtc.text = ctext;
          gtc.evidenceRefs = refs;
          gtc.confidence = conf;
          claims.add(gtc);
        }
        i++;
    ...
    if (m.get("sections") instanceof List) {
      ...
      for (Object raw : (List<?>) m.get("sections")) {
        if (!(raw instanceof Map)) {
          v.violations.add("sections[" + i + "] must be an object.");
          i++;
          continue;
        }
        ...
        if (!java.util.Arrays.asList(
                "judgement", "reason", "variation", "training", "profile", "evidence-note")
            .contains(section)) v.violations.add("sections[" + i + "].section is invalid.");
        if (md.isEmpty()) v.warnings.add("sections[" + i + "] has empty markdown.");
        if (scids == null)
          v.violations.add("sections[" + i + "].claimIds must be an array of strings.");
        if (scids != null)
          for (String cid : scids)
            if (!claimIds.contains(cid))
              v.violations.add("sections[" + i + "] references unknown claim " + cid);
        if (!id.isEmpty()
            && java.util.Arrays.asList(
                    "judgement", "reason", "variation", "training", "profile", "evidence-note")
                .contains(section)
            && scids != null) {
          GroundedTeachingSection gts = new GroundedTeachingSection();
          gts.id = id;
          gts.section = section;
          gts.markdown = md;
          gts.claimIds = scids;
    ...
    if (claims.isEmpty())
      v.warnings.add(
          "Structured result has no validated claims; quality gate will fall back to markdown scanning.");
    if (v.violations.isEmpty()) {
      GroundedTeachingOutput out = new GroundedTeachingOutput();
      out.headline = str(m.get("headline"));
      out.summary = str(m.get("summary"));
      out.confidence = str(m.get("confidence"));
      out.finalMarkdown = str(m.get("finalMarkdown"));
      out.claims = claims;
      out.sections = sections;
      out.drills = drills != null ? drills : new ArrayList<>();
      out.followupQuestions = fqs != null ? fqs : new ArrayList<>();
      v.result = out;
    }
  ...
  static featurecat.lizzie.teacher.knowledge.JsonKnowledgeLoader JsonKnowledgeLoader_min() {
    return new featurecat.lizzie.teacher.knowledge.JsonKnowledgeLoader();
  }

  public static String buildStructuredTeachingInstruction() {
    return "为了便于 GoAgent 校验证据，最终答案应能被拆成结构化 claims。\n"
        + "如果当前 LLM 支持结构化 JSON，请按 "
        + GOAGENT_GROUNDING_JSON_MARKER
        + " 输出 GroundedTeachingOutput；否则输出自然语言，但每个关键结论必须可回指到 KataGo、棋盘、知识库或学生画像证据。\n"
