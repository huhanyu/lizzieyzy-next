...
/**
 * 对齐 GoAgent 的 qualityGate.ts（87 行）全量： runTeacherQualityGate（结构化输出验证 + verifyTeacherMarkdown +
 * claimVerification 聚合， failOnWarning 可配置，产出 note）+ appendTeacherQualityGateNote。
 */
public final class QualityGate {
  ...
  public static TeacherQualityGateResult runTeacherQualityGate(
      String markdown, TeachingEvidenceBuilder.TeachingEvidence evidence, boolean failOnWarning) {
    TeacherQualityGateResult r = new TeacherQualityGateResult();
    // 结构化输出验证
    ClaimVerifier.GroundedTeachingOutput structured = null;
    String jsonCandidate = ClaimVerifier.normalizeJsonCandidate(markdown);
    if (jsonCandidate != null) {
      try {
        Object parsed =
            featurecat.lizzie.teacher.knowledge.JsonKnowledgeLoader.parse(jsonCandidate);
        ClaimVerifier.StructuredTeachingValidation sv =
            ClaimVerifier.validateGroundedTeachingResult(parsed);
        r.structuredWarnings = sv.warnings;
        r.structuredViolations = sv.violations;
        if (sv.ok) structured = sv.result;
      } catch (RuntimeException parseError) {
        r.structuredViolations =
            Collections.singletonList("Structured grounding JSON is malformed.");
      }
    }
    ...
    if (evidence != null) {
      ...
      cv =
          (structured != null && structured.claims != null && !structured.claims.isEmpty())
      ...
      warnings.addAll(mdv.warnings);
      warnings.addAll(cv.warnings);
      violations.addAll(mdv.violations);
      violations.addAll(cv.violations);
    } else {
      warnings.add(
          "No TeachingEvidence was provided; quality gate could not verify coordinates, numbers, PV, joseki names, or confidence wording.");
    }
    ...
    if (evidence != null && mdv != null)
      noteParts.add(TeachingEvidenceBuilder.buildVerificationNote(mdv, evidence, "zh-CN"));
    if (evidence != null && cv != null) noteParts.add(ClaimVerifier.buildClaimVerificationNote(cv));
    if (noteParts.isEmpty()) {
      List<String> issue = new ArrayList<>(r.violations);
      issue.addAll(r.warnings);
      String sample =
          issue.size() > 3 ? String.join("；", issue.subList(0, 3)) : String.join("；", issue);
      noteParts.add(
          "> GoAgent 质量门禁："
              + (r.ok ? "通过" : "未通过")
              + "。"
              + (sample.isEmpty() ? "没有可校验的证据。" : sample));
    }
    r.note = String.join("\n", noteParts);
    return r;
  }

  public static String appendTeacherQualityGateNote(
      String markdown, TeacherQualityGateResult gate) {
