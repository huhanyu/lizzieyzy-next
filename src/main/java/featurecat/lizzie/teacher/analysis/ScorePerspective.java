...
/**
 * 对齐 GoAgent 的 scorePerspective.ts：胜率/目差视角校准工具。 KataGo 约定：scoreLead
 * 为"黑方领先目数"（black-positive）；winrate 为"当前轮到走棋方胜率"。 提供把数据转到"指定方视角"的工具，避免手拍视角导致符号错乱。
 */
public final class ScorePerspective {
  ...
  public static ScoreSummary scoreSummaryFromBlackLead(
      Double blackScoreLead, String perspectiveColor) {
    double blackLead =
        (blackScoreLead != null && Double.isFinite(blackScoreLead)) ? blackScoreLead : 0;
    ...
    s.perspectiveScoreLead =
        perspectiveColor != null
            ? round(scoreLeadForColor(blackLead, perspectiveColor.equals("B")), 2)
            : null;
