...
public final class TeacherPersona {
  ...
  public enum TerminologyDensity {
    LOW,
    MEDIUM,
    HIGH
  }

  public enum ExplanationPace {
    BRIEF,
    STANDARD,
    DETAILED
  }

  public enum VariationDetail {
    FEW,
    MODERATE,
    MANY
  }

  public static final java.util.Map<Style, String> TEACHER_STYLE_LABELS =
      java.util.Map.of(
          Style.BALANCED,
          "平衡自然",
          Style.RIGOROUS,
          ...
          Style.HUMOROUS,
          "风趣幽默");
  public static final java.util.Map<AgeRange, String> STUDENT_AGE_LABELS =
      java.util.Map.of(
          AgeRange.UNKNOWN,
          "未指定年龄",
          AgeRange.CHILD,
  ...
  public static final class TeacherPersonaInput {
    public Level level = Level.INTERMEDIATE;
    public Rank rank;
    public Integer exactAge;
    public AgeRange ageRange = AgeRange.UNKNOWN;
  ...
  static String levelInstruction(Level level) {
    if (level == null) return levelInstruction(Level.INTERMEDIATE);
    if (level == Level.BEGINNER)
      return "当前用户是入门水平。少用术语，优先讲“这手下一次怎么想”、气、断点、连接、先后手和一两个可执行提醒。PV 不要展开太长。";
  ...
  static String rankInstruction(Rank rank) {
    ...
    switch (rank) {
      case SUB1D:
        return "当前用户段位：1k以下（级位）。优先讲可执行的下一手判断、方向和基本形状，避免堆变化。";
      case D1:
        return "当前用户段位：1d。可以比较候选点、方向和厚薄转换。";
      case D2:
        return "当前用户段位：2d。可以讲关键变化、方向选择和攻防先后。";
      case D3:
        return "当前用户段位：3d。可以讲更细的目差、PV 分歧和中盘攻防节奏。";
      case D4:
        return "当前用户段位：4d。可以讲转换价值、局部收益与全局厚薄的权衡。";
      case D5:
        return "当前用户段位：5d。可以讲高阶次序、交换价值、全局转换和局部读秒级判断。";
      case D6:
        return "当前用户段位：6d。可以更直接比较复杂变化、劫材价值和战略转换。";
      case D7:
        return "当前用户段位：7d。可以讲深层次序、细微目差和胜率风险控制。";
      case D8:
        return "当前用户段位：8d。可以讲接近职业训练的候选点取舍和高精度转换。";
      case D9:
        return "当前用户段位：9d。可以使用高密度术语、复杂 PV 对照和职业级证据链复盘。";
    }
    return "";
  }
  ...
  public static String buildTeacherPersonaInstruction(TeacherPersonaInput input) {
    TeacherPersonaInput safeInput = input == null ? new TeacherPersonaInput() : input;
    java.util.List<String> parts = new java.util.ArrayList<>();
    parts.add("【风格设置】");
    parts.add(levelInstruction(safeInput.level));
    String r = rankInstruction(safeInput.rank);
    if (!r.isEmpty()) parts.add(r);
    String e = exactAgeInstruction(safeInput.exactAge);
    if (!e.isEmpty()) parts.add(e);
    parts.add(ageInstruction(safeInput.ageRange));
    parts.add(styleInstruction(safeInput.style));
    parts.add(densityInstruction(safeInput.terminologyDensity));
    parts.add(paceInstruction(safeInput.explanationPace));
    parts.add(variationInstruction(safeInput.variationDetail));
    parts.add(
