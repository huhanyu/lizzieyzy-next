...
/**
 * 对齐 GoAgent knowledge/josekiRecognizer.ts + josekiTrie.ts： 定式卡数据模型 + 识别（相对坐标/角落/文本信号/Trie 序列匹配）。
 */
public final class JosekiRecognizer {

  private JosekiRecognizer() {}

  public enum JosekiConfidence {
    strong,
    medium,
    weak
  }

  public enum JosekiCorner {
    SW,
    SE,
    NW,
    NE
  ...
  static String[] textHit(String text, String[] signals) {
    ...
    for (String s : signals)
      if (s != null && !s.isEmpty() && lower.contains(s.toLowerCase())) hits.add(s);
  ...
  static class ScoreResult {
    ...
    List<JosekiNextMove> nextMoves = new ArrayList<>();
  }

  static ScoreResult scoreCard(
      JosekiPatternCard card,
      JosekiRecognitionQuery query,
      JosekiCorner corner,
    ...
    if (requiredHits.size() == card.requiredRelativeStones.length
        && card.requiredRelativeStones.length > 1) {
      r.score += 5;
      r.evidence.add("all required local stones found");
    }
    String[] signals =
        textHit(signalText, concatArr(new String[] {card.name, card.family}, card.sequenceSignals));
    if (signals.length > 0) {
      r.score += Math.min(6, signals.length * 2);
      r.evidence.add(
          "text/signal hits: "
              + String.join(", ", Arrays.copyOf(signals, Math.min(3, signals.length))));
    }
    for (JosekiNextMove mv : card.commonNextMoves) {
      mv.gtpMove =
          (mv.relativeMove != null && mv.relativeMove.contains("-"))
              ? gtpFromRelative(mv.relativeMove, corner, query.boardSize)
              : null;
    }
    JosekiNextMove nextHit = null;
    for (JosekiNextMove mv : card.commonNextMoves)
      if (mv.gtpMove != null && moveSet.contains(mv.gtpMove)) {
        nextHit = mv;
        break;
      }
    if (nextHit != null) {
      r.score += 4;
      r.evidence.add("candidate/PV matches common next move " + nextHit.gtpMove);
    }
    if (query.moveNumber <= 80) r.score += 2;
    else if (query.moveNumber <= 120) r.score += 1;
    else r.score -= 2;
    if (card.requiredRelativeStones.length <= 1 && signals.length == 0 && nextHit == null)
      r.score -= 4;
  ...
  public static List<RecognizedJosekiPattern> recognizeJosekiPatterns(
      JosekiRecognitionQuery query) {
    ...
    List<JosekiMoveLike> recent =
        query.recentMoves == null
            ? new ArrayList<>()
            : query.recentMoves.subList(
                Math.max(0, query.recentMoves.size() - 40), query.recentMoves.size());
    ...
    for (JosekiCorner c : JosekiCorner.values()) {
      corners.put(c, new HashSet<>());
      rawCornerMoves.put(c, new ArrayList<>());
    }
    Set<String> seen = new HashSet<>();
    ...
    for (String m : new String[] {query.actualMove, query.bestMove}) {
      String n = normalizeMove(m);
      if (n != null) moveSet.add(n);
    }
    for (String m : safeArr(query.candidateMoves)) {
      String n = normalizeMove(m);
      if (n != null) moveSet.add(n);
    }
    for (String m : safeArr(query.principalVariation)) {
      String n = normalizeMove(m);
      if (n != null) moveSet.add(n);
    }
    ...
    for (JosekiPatternCard card : cards) {
      ...
      for (JosekiCorner corner : JosekiCorner.values()) {
        ...
        for (JosekiTrie.JosekiTrieMatch tm : trieMatches)
          if (tm.cardId.equals(card.id) && tm.corner == corner) {
            trieMatch = tm;
            break;
          }
        if (trieMatch != null) {
          sr.score +=
              trieMatch.confidence == JosekiTrie.JosekiTrieConfidence.strong
                  ? 6
                  : trieMatch.confidence == JosekiTrie.JosekiTrieConfidence.medium ? 3 : 1;
          sr.evidence.add("sequence-trie:" + trieMatch.safeWording);
          for (int i = 0; i < Math.min(2, trieMatch.evidence.length); i++)
            sr.evidence.add(trieMatch.evidence[i]);
        }
        if (sr.score < 9) continue;
        RecognizedJosekiPattern pat = new RecognizedJosekiPattern();
        pat.id = card.id;
        pat.name = card.name;
        pat.family = card.family;
        pat.confidence =
            confidenceScore(sr.score) == 2
                ? JosekiConfidence.strong
        ...
        ev.add(
            "cornerMoves="
                + String.join(
                    ...
                    rawCornerMoves
                        .get(corner)
                        .subList(0, Math.min(8, rawCornerMoves.get(corner).size()))));
        if (ev.size() > 10) ev = new ArrayList<>(ev.subList(0, 10));
        pat.evidence = ev.toArray(new String[0]);
        pat.sourceRefs =
            card.sourceRefs != null ? card.sourceRefs : new String[] {"goagent-curated-original"};
    ...
    results.sort(
        (a, b) ->
            Double.compare(b.score, a.score) != 0
                ...
                : Integer.compare(b.variationCount, a.variationCount) != 0
                    ? Integer.compare(b.variationCount, a.variationCount)
  ...
  static String[] safeArr(String[]... arrs) {
    List<String> l = new ArrayList<>();
    for (String[] a : arrs) if (a != null) for (String x : a) if (x != null) l.add(x);
    return l.toArray(new String[0]);
  }

  static String[] nonNull(String... a) {
    List<String> l = new ArrayList<>();
  ...
  public static String formatJosekiPatternsForPrompt(List<RecognizedJosekiPattern> patterns) {
    ...
    for (int idx = 0; idx < n; idx++) {
      ...
      for (int k = 0; k < Math.min(4, p.commonNextMoves.size()); k++) {
        ...
        next.append(mv.gtpMove != null ? mv.gtpMove : mv.relativeMove)
            .append(": ")
            .append(mv.label)
