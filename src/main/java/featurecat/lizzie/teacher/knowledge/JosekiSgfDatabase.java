...
/**
 * 对齐 GoAgent knowledge/josekiSgfDatabase.ts： 从打包的 SGF 定式库（pachi + josekle）解析出定式线，构建
 * JosekiPatternCard。 不依赖 electron/node：从 classpath 读取 resources/knowledge/joseki-sgf/ 下 SGF。
 */
public final class JosekiSgfDatabase {
  ...
  static {
    ...
    josekle.id = "josekle-mit-dictionary";
    josekle.dir = "josekle";
    ...
    aiJoseki.id = "ai-joseki-daquan";
    aiJoseki.dir = "ai-joseki";
  ...
  static class Point {
    ...
    Point(int x, int yFromTop) {
      ...
      this.relative =
          Math.min(x + 1, BOARD_SIZE - x)
              + "-"
              + Math.min(yFromBottom + 1, BOARD_SIZE - yFromBottom);
    }
  }
  ...
  private static void remember(
      List<List<SgfMove>> paths, Set<String> seen, List<SgfMove> current, int maxMoves) {
  ...
  static String labelForNextMove(SgfMove move, BundledJosekiSource source) {
    ...
    if (c.contains("<dont>")
        || c.contains("<don't>")
        || c.contains("<bad>")
        || c.contains("<avoid>")) tags.add("avoid/deviation response");
  ...
  static List<JosekiRecognizer.JosekiPatternCard> buildCardsForFile(
      BundledJosekiSource source, String filename, String content) {
    ...
    for (List<SgfMove> path : paths) {
      ...
      for (int prefixLength = 2; prefixLength <= maxPrefix; prefixLength++) {
        ...
        if (entry == null) {
          ...
          entry.id = id;
          entry.source = source;
          entry.family = familyForFile(source, filename, path.get(0));
          entry.file = filename;
          entry.prefixMoves = new ArrayList<>(prefix);
        ...
        if (!entry.nextMoves.containsKey(next.relative)) {
          ...
          if (nm.condition != null && nm.condition.length() > 120)
            nm.condition = nm.condition.substring(0, 120);
    ...
    for (CardAccumulator entry : byPrefix.values()) {
      ...
      card.recognition =
          "Bundled SGF database match from "
      ...
      card.wrongThinking =
          "Do not memorize this branch mechanically; joseki choice depends on ladder status, neighboring stones, sente value, and whole-board direction.";
      card.correctThinking =
          "Identify the corner family, compare the SGF branch candidates with KataGo candidates, then explain whether the player should continue the local sequence, tenuki, simplify, or choose a different direction.";
      card.drillPrompt =
          "Cover the next move and ask: which branch keeps the right direction in this whole-board position, and what outside stones would change the answer?";
      out.add(card);
    }
    return out;
  }
  ...
  private static List<String> listSgfFiles(String dir) {
    ...
    if (dir.contains("/pachi/"))
      return Arrays.asList("joseki_33.sgf", "joseki_34.sgf", "joseki_44.sgf", "joseki_54.sgf");
  ...
  public static class BundledJosekiSummary {
    public int sourceCount, cardCount;
  ...
  public static BundledJosekiSummary summarizeBundledJosekiSgfCards() {
    ...
    for (JosekiRecognizer.JosekiPatternCard c : cards)
      if (c.sourceRefs != null) for (String s2 : c.sourceRefs) srcs.add(s2);
