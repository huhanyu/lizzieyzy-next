...
/**
 * 对齐 GoAgent knowledge/josekiTrie.ts：把定式卡的序列（相对坐标）与查询的角落序列做前缀匹配， 给出 strong/medium/weak 置信度与
 * safeWording。
 */
public final class JosekiTrie {
  ...
  public enum JosekiTrieConfidence {
    strong,
    medium,
    weak
  }
  ...
  static class SequenceToken {
    Character color;
    String relative;
  }

  static class CornerMoveToken extends SequenceToken {
    String rawMove;
  }
  ...
  static List<SequenceToken[]> cardSequences(JosekiRecognizer.JosekiPatternCard card) {
    ...
    Set<String> req =
        new LinkedHashSet<>(
            Arrays.asList(
                card.requiredRelativeStones != null ? card.requiredRelativeStones : new String[0]));
    if (out.isEmpty() && req.size() >= 2) {
      SequenceToken[] toks = new SequenceToken[req.size()];
      int i = 0;
      for (String r : req) {
        toks[i++] = new SequenceToken();
        toks[i - 1].relative = r;
      }
      out.add(toks);
    }
  ...
  static SequenceToken[] parseSequenceTokens(String value) {
    ...
    while (m.find()) {
      ...
      t.color =
          (m.group(1) != null && (m.group(1).equals("B") || m.group(1).equals("W")))
              ? m.group(1).charAt(0)
              : null;
  ...
  static Map<JosekiRecognizer.JosekiCorner, List<CornerMoveToken>> queryCornerSequences(
      JosekiRecognizer.JosekiRecognitionQuery query) {
    Map<JosekiRecognizer.JosekiCorner, List<CornerMoveToken>> result =
        new EnumMap<>(JosekiRecognizer.JosekiCorner.class);
    for (JosekiRecognizer.JosekiCorner c : JosekiRecognizer.JosekiCorner.values())
      result.put(c, new ArrayList<>());
    List<JosekiRecognizer.JosekiMoveLike> recent =
        query.recentMoves == null
            ? new ArrayList<>()
            : query.recentMoves.subList(
                Math.max(0, query.recentMoves.size() - 60), query.recentMoves.size());
    int idx = 0;
  ...
  static ScoreOut scoreSequence(SequenceToken[] cardTokens, CornerMoveToken[] queryTokens) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      o.prefixLength++;
      if (cardTokens[i].color == null
          || queryTokens[i].color == null
          || cardTokens[i].color == queryTokens[i].color) o.colorHits++;
    }
    ...
    o.exactOrderMatch =
        o.prefixLength >= Math.min(Math.min(cardTokens.length, queryTokens.length), 4);
    o.colorConsistent =
        o.prefixLength == 0
            ? false
            : o.colorHits >= Math.max(1, (int) Math.floor(o.prefixLength * 0.75));
    return o;
  }

  static JosekiTrieConfidence confidenceFrom(
      double score, boolean exactOrderMatch, boolean colorConsistent) {
  ...
  static String wordingFor(
      JosekiTrieConfidence c, boolean exactOrderMatch, boolean kataGoSupports) {
  ...
  public static List<JosekiTrieMatch> recognizeJosekiTrie(
      List<JosekiRecognizer.JosekiPatternCard> cards,
      JosekiRecognizer.JosekiRecognitionQuery query) {
    if (query.boardSize != 19 || cards.isEmpty()) return new ArrayList<>();
    Map<JosekiRecognizer.JosekiCorner, List<CornerMoveToken>> byCorner =
        queryCornerSequences(query);
    Set<String> continuationRelatives =
        normalizeRelativeMoves(
            concatArr(
                new String[] {query.actualMove, query.bestMove},
                safe(query.candidateMoves),
                safe(query.principalVariation)),
            query.boardSize);
    ...
    for (JosekiRecognizer.JosekiPatternCard card : cards) {
      ...
      for (JosekiRecognizer.JosekiCorner corner : JosekiRecognizer.JosekiCorner.values()) {
        ...
        for (SequenceToken[] seq : sequences) {
          ...
          boolean kataGoSupports =
              nextToken != null && continuationRelatives.contains(nextToken.relative);
          double score =
              sc.prefixLength * 5
                  + sc.colorHits * 2
                  + sc.relativeHits * 2
                  + (kataGoSupports ? 5 : 0)
                  - Math.min(6, sc.tenukiCount);
          if (score < 8) continue;
          ...
          m.cardId = card.id;
          m.family = card.family;
          m.corner = corner;
          m.prefixLength = sc.prefixLength;
          m.relativeHits = sc.relativeHits;
          m.colorHits = sc.colorHits;
          m.tenukiCount = sc.tenukiCount;
          m.exactOrderMatch = sc.exactOrderMatch;
          m.colorConsistent = sc.colorConsistent;
          m.kataGoSupportsContinuation = kataGoSupports;
          m.confidence = conf;
          m.score = Math.round(score * 10) / 10.0;
          m.safeWording = wordingFor(conf, sc.exactOrderMatch, kataGoSupports);
          m.evidence =
              new String[] {
                "sequencePrefix=" + sc.prefixLength,
                "relativeHits=" + sc.relativeHits,
                "colorHits=" + sc.colorHits,
                "tenukiCount=" + sc.tenukiCount,
                "kataGoSupportsContinuation=" + kataGoSupports
              };
          matches.add(m);
        }
      }
    ...
    matches.sort(
        (a, b) ->
            Double.compare(b.score, a.score) != 0
                ...
                : Integer.compare(b.prefixLength, a.prefixLength) != 0
                    ? Integer.compare(b.prefixLength, a.prefixLength)
  ...
  public static String josekiTrieMatchSummary(JosekiTrieMatch match) {
    return match.safeWording
        + "：prefix="
        + match.prefixLength
        + "，颜色一致="
        + (match.colorConsistent ? "是" : "否")
        + "，KataGo续手支持="
        + (match.kataGoSupportsContinuation ? "是" : "否")
        + "。";
  }
}
