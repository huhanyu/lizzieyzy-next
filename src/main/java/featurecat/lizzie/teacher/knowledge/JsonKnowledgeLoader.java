...
public final class JsonKnowledgeLoader {
  ...
  // ---- JSON parsing ----
  // Reuse the project's maintained parser instead of accepting malformed JSON or mishandling
  // escaped Unicode and numeric values in the bundled knowledge resources.
  ...
  private static Object toJavaValue(Object value) {
    ...
    if (value instanceof JSONObject) {
      ...
      for (String key : object.keySet()) converted.put(key, toJavaValue(object.get(key)));
      return converted;
    }
    if (value instanceof JSONArray) {
      ...
      for (int index = 0; index < array.length(); index++) {
        converted.add(toJavaValue(array.get(index)));
      }
      return converted;
    }
    return value;
  ...
  static String[] asStrArray(Object o) {
    ...
    if (o instanceof List) {
      ...
      for (int i = 0; i < l.size(); i++) out[i] = l.get(i) == null ? null : l.get(i).toString();
      return out;
    }
  ...
  private static List<JosekiRecognizer.JosekiPatternCard> parseJosekiPatternCards() {
    ...
    for (Object item : (List<?>) parsed) {
      ...
      Object bs = m.get("boardSize");
      card.boardSize = bs == null ? 19 : Integer.parseInt(bs.toString());
      ...
      Object vc = m.get("variationCount");
      card.variationCount = vc == null ? 0 : Integer.parseInt(vc.toString());
      ...
      if (cnm instanceof List)
        for (Object o : (List<?>) cnm) {
      ...
      if (vars instanceof List)
        for (Object o : (List<?>) vars) card.variations.add(o == null ? "" : o.toString());
      out.add(card);
    }
    return out;
  }
  ...
  public static List<ElitePatternCard> loadEliteCards() {
    return EliteCardsHolder.CARDS;
  }

  private static final class EliteCardsHolder {
    private static final List<ElitePatternCard> CARDS =
        Collections.unmodifiableList(parseEliteCards());
  }

  private static List<ElitePatternCard> parseEliteCards() {
    ...
    for (String f : files) {
      ...
      for (Object item : (List<?>) parsed) {
        ...
        c.id = asStr(m.get("id"));
        c.title = asStr(m.get("title"));
        c.category = asStr(m.get("category"));
        c.patternType = asStr(m.get("patternType"));
        c.scope = asStr(m.get("scope"));
        c.phase = asStrArray(m.get("phase"));
        c.regions = asStrArray(m.get("regions"));
        c.levels = asStrArray(m.get("levels"));
        c.tags = asStrArray(m.get("tags"));
        c.aliases = asStrArray(m.get("aliases"));
        c.triggerSignals = asStrArray(m.get("triggerSignals"));
        c.negativeSignals = asStrArray(m.get("negativeSignals"));
        c.katagoSignals = asStrArray(m.get("katagoSignals"));
        c.sourceRefs = asStrArray(m.get("sourceRefs"));
        c.recognition = asStr(m.get("recognition"));
        c.wrongThinking = asStr(m.get("wrongThinking"));
        c.correctThinking = asStr(m.get("correctThinking"));
        c.drillPrompt = asStr(m.get("drillPrompt"));
        c.sourceQuality = asStr(m.get("sourceQuality"));
        Object cb = m.get("confidenceBoost");
        c.confidenceBoost = cb == null ? 0 : Double.parseDouble(cb.toString());
        out.add(c);
      }
    }
    return out;
  ...
  public static List<LocalShapeGeometryMatcher.ProblemEntry> loadTrainingProblems() {
    return TrainingProblemsHolder.PROBLEMS;
  }

  private static final class TrainingProblemsHolder {
    private static final List<LocalShapeGeometryMatcher.ProblemEntry> PROBLEMS =
        Collections.unmodifiableList(parseTrainingProblems());
  }

  private static List<LocalShapeGeometryMatcher.ProblemEntry> parseTrainingProblems() {
    List<LocalShapeGeometryMatcher.ProblemEntry> out = new ArrayList<>();
    String json = readResource("knowledge/training-catalog.json");
    if (json == null) return out;
    ...
    for (String key : new String[] {"lifeDeathProblems", "tesujiProblems"}) {
      ...
      for (Object item : (List<?>) arr) {
        ...
        pe.id = asStr(prob.get("id"));
        pe.title = asStr(prob.get("title"));
        pe.region = asStr(prob.get("region"));
        pe.difficulty = asStr(prob.get("difficulty"));
        pe.objective = asStr(prob.get("objective"));
        pe.sourceKind = asStr(prob.get("sourceKind"));
        ...
        if (init instanceof List)
          for (Object st : (List<?>) init) {
            if (st instanceof Map) {
              Map<?, ?> sm = (Map<?, ?>) st;
              LocalPatternMatcher.BoardSnapshotStone bs =
                  new LocalPatternMatcher.BoardSnapshotStone();
              bs.point =
                  sm.get("point") != null
                      ? sm.get("point").toString()
                      : (sm.get("gtp") != null ? sm.get("gtp").toString() : null);
        ...
        if (cm instanceof List)
          for (Object mv : (List<?>) cm) {
            LocalShapeGeometryMatcher.TrainingMove tm =
                new LocalShapeGeometryMatcher.TrainingMove();
            if (mv instanceof Map) {
              Map<?, ?> mm = (Map<?, ?>) mv;
              tm.move =
                  mm.get("move") != null
                      ? mm.get("move").toString()
                      : (mm.get("gtp") != null ? mm.get("gtp").toString() : null);
            } else tm.move = mv != null ? mv.toString() : null;
            if (tm.move != null) pe.correctMoves.add(tm);
          }
        Object fm = prob.get("failureMoves");
        if (fm instanceof List)
          for (Object mv : (List<?>) fm) {
            LocalShapeGeometryMatcher.TrainingMove tm =
                new LocalShapeGeometryMatcher.TrainingMove();
            if (mv instanceof Map) {
              Map<?, ?> mm = (Map<?, ?>) mv;
              tm.move = asStr(mm.get("move"));
              tm.why = asStr(mm.get("why"));
            } else tm.move = mv != null ? mv.toString() : null;
  ...
  public static class JosekiLineEntry {
    ...
    public List<String> phase = new ArrayList<>(),
        ...
        tags = new ArrayList<>(),
        normalizedFeatures = new ArrayList<>(),
        relativeSequence = new ArrayList<>(),
        decisionRules = new ArrayList<>(),
        commonMistakes = new ArrayList<>(),
        trainingFocus = new ArrayList<>(),
        patternCardIds = new ArrayList<>();
    public List<Branch> branches = new ArrayList<>();

    public static class Branch {
      public String name, whenToChoose, warning;
    }
  }

  public static List<JosekiLineEntry> loadJosekiLines() {
    return JosekiLinesHolder.LINES;
  }

  private static final class JosekiLinesHolder {
    private static final List<JosekiLineEntry> LINES =
        Collections.unmodifiableList(parseJosekiLines());
  }

  private static List<JosekiLineEntry> parseJosekiLines() {
    ...
    for (Object item : (List<?>) arr) {
      ...
      e.id = asStr(m.get("id"));
      e.title = asStr(m.get("title"));
      e.family = asStr(m.get("family"));
      e.katagoEraJudgement = asStr(m.get("katagoEraJudgement"));
      e.sourceKind = asStr(m.get("sourceKind"));
      e.phase = asStrList(m.get("phase"));
      e.levels = asStrList(m.get("levels"));
      e.tags = asStrList(m.get("tags"));
      e.normalizedFeatures = asStrList(m.get("normalizedFeatures"));
      e.relativeSequence = asStrList(m.get("relativeSequence"));
      e.decisionRules = asStrList(m.get("decisionRules"));
      e.commonMistakes = asStrList(m.get("commonMistakes"));
      e.trainingFocus = asStrList(m.get("trainingFocus"));
      e.patternCardIds = asStrList(m.get("patternCardIds"));
      Object br = m.get("branches");
      if (br instanceof List)
        for (Object b : (List<?>) br) {
          if (b instanceof Map) {
            Map<?, ?> bm = (Map<?, ?>) b;
            JosekiLineEntry.Branch bb = new JosekiLineEntry.Branch();
            bb.name = asStr(bm.get("name"));
            bb.whenToChoose = asStr(bm.get("whenToChoose"));
            bb.warning = asStr(bm.get("warning"));
            e.branches.add(bb);
          }
        }
      out.add(e);
  ...
  public static List<LocalPatternMatcher.ShapePatternCard> loadShapePatternCards() {
    return ShapePatternCardsHolder.CARDS;
  }

  private static final class ShapePatternCardsHolder {
    private static final List<LocalPatternMatcher.ShapePatternCard> CARDS =
        Collections.unmodifiableList(parseShapePatternCards());
  }

  private static List<LocalPatternMatcher.ShapePatternCard> parseShapePatternCards() {
    List<LocalPatternMatcher.ShapePatternCard> out = new ArrayList<>();
    String json = readResource("knowledge/shape-pattern-cards-v1.json");
    if (json == null) return out;
    Object root = parse(json);
    if (root instanceof List)
      for (Object item : (List<?>) root) {
        if (item instanceof Map) out.add(parseShapeCard((Map<?, ?>) item));
      }
    else if (root instanceof Map) {
      Object cards = ((Map<?, ?>) root).get("cards");
  ...
  private static LocalPatternMatcher.ShapePatternCard parseShapeCard(Map<?, ?> m) {
    ...
    c.id = asStr(m.get("id"));
    c.title = asStr(m.get("title"));
    c.shapeType = asStr(m.get("shapeType"));
    c.category = asStr(m.get("category"));
    c.anchorRole = asStr(m.get("anchorRole"));
    c.sourceQuality = asStr(m.get("sourceQuality"));
    c.minScore =
        m.get("minScore") instanceof Number ? ((Number) m.get("minScore")).doubleValue() : null;
    c.phase = asStrList(m.get("phase"));
    c.regions = asStrList(m.get("regions"));
    c.tags = asStrList(m.get("tags"));
    c.sourceRefs = asStrList(m.get("sourceRefs"));
    Object pts = m.get("points");
    if (pts instanceof List)
      for (Object p : (List<?>) pts) {
        if (p instanceof Map) {
          Map<?, ?> pm = (Map<?, ?>) p;
          LocalPatternMatcher.LocalPatternPoint lp = new LocalPatternMatcher.LocalPatternPoint();
          lp.dx = (int) asNum(pm.get("dx"));
          lp.dy = (int) asNum(pm.get("dy"));
          lp.state = LocalPatternMatcher.LocalPatternPointState.valueOf(asStr(pm.get("state")));
          lp.required = pm.get("required") instanceof Boolean ? (Boolean) pm.get("required") : true;
          c.points.add(lp);
        }
      }
    Object cons = m.get("constraints");
    if (cons instanceof List)
      for (Object cn : (List<?>) cons) {
        if (cn instanceof Map) {
          Map<?, ?> cm = (Map<?, ?>) cn;
          LocalPatternMatcher.LocalPatternConstraint lc =
              new LocalPatternMatcher.LocalPatternConstraint();
          lc.type = asStr(cm.get("type"));
          lc.value = (int) asNum(cm.get("value"));
          c.constraints.add(lc);
        }
      }
    Object anti = m.get("antiPatterns");
    if (anti instanceof List)
      for (Object a : (List<?>) anti) {
        if (a instanceof Map) {
          Map<?, ?> am = (Map<?, ?>) a;
          LocalPatternMatcher.LocalPatternPoint lp = new LocalPatternMatcher.LocalPatternPoint();
          lp.dx = (int) asNum(am.get("dx"));
          lp.dy = (int) asNum(am.get("dy"));
          lp.state = LocalPatternMatcher.LocalPatternPointState.valueOf(asStr(am.get("state")));
          c.antiPatterns.add(lp);
        }
      }
    Object t = m.get("teaching");
    if (t instanceof Map) {
      Map<?, ?> tm = (Map<?, ?>) t;
      c.teaching = new LocalPatternMatcher.Teaching();
      c.teaching.recognition = asStr(tm.get("recognition"));
      c.teaching.wrongThinking = asStr(tm.get("wrongThinking"));
      c.teaching.correctThinking = asStr(tm.get("correctThinking"));
      c.teaching.drillPrompt = asStr(tm.get("drillPrompt"));
    }
    return c;
  }
}
