...
public final class KatagoShapeFeatures {
  ...
  public static List<KataGoShapeFeature> extract(KataGoShapeFeatureInput in) {
    ...
    String best =
        in.candidateMoves != null && !in.candidateMoves.isEmpty() ? in.candidateMoves.get(0) : null;
    ...
    List<String> related =
        uniq(
            new ArrayList<>() {
              {
                if (actual != null) add(actual);
                if (best != null) add(best);
                if (in.principalVariation != null)
                  addAll(in.principalVariation.stream().limit(6).toList());
              }
            });
    ...
    if (scoreLoss >= 1.5 && localDistance <= 2 && !gamePhase.equals("opening")) {
      features.add(
          mk(
              "katago-local-candidate-shape-detail",
              "local_shape_detail",
              scoreLoss >= 4 ? "strong" : "medium",
              18 + Math.min(8, scoreLoss * 2),
              related,
    ...
    if (scoreLoss >= 1.5 && localDistance >= 6) {
      features.add(
          mk(
              "katago-global-vs-local-shape-choice",
              "local_vs_global_shape",
              scoreLoss >= 4 ? "strong" : "medium",
              16 + Math.min(8, scoreLoss * 1.5),
              related,
    ...
    if ((in.principalVariation != null && in.principalVariation.size() >= 6) && scoreLoss >= 1) {
      features.add(
          mk(
              "katago-pv-supported-shape-line",
              "pv_supported_shape",
              in.principalVariation.size() >= 8 ? "medium" : "weak",
              12 + Math.min(6, in.principalVariation.size()),
              related,
  ...
  static KataGoShapeFeature mk(
    ...
    f.score = score;
    f.relatedMoves = related;
    f.recognition = recognition;
    f.wrongThinking = wrongThinking;
    f.correctThinking = correctThinking;
    f.drillPrompt = drillPrompt;
