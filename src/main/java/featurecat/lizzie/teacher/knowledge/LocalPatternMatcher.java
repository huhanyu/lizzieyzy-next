...
/**
 * 对齐 GoAgent knowledge/localPatternMatcher.ts（275 行）：通用局部模式匹配引擎。 用 ShapePatternCard 的
 * points/constraints/antiPatterns + 8 种旋转/镜像变换，在当前棋盘上以 anchor 为中心匹配棋形。 与 MotifRecognizer 的 elite
 * 卡快速路径互补：这里更通用，支持任意 ShapePatternCards（elite/joseki/pattern-cards）。
 */
public final class LocalPatternMatcher {

  private LocalPatternMatcher() {}

  public enum LocalPatternPointState {
    friendly,
    enemy,
    empty,
    black,
  ...
  public enum LocalPatternConfidence {
    strong,
    medium,
    weak
  }

  public static class LocalPatternPoint {
    public int dx, dy;
    public LocalPatternPointState state;
    public boolean required = true;
  }

  public static class LocalPatternConstraint {
    public String type;
    public int value;
  }
  ...
  public static class BoardSnapshotStone {
    public String point;
    public String color;
  }

  public static class LocalWindow {
    public String anchor;
    public List<BoardSnapshotStone> stones = new ArrayList<>();
  }

  public static class LocalPatternMatch {
    public ShapePatternCard card;
    public String confidence, anchor, transform, perspective;
  ...
  static class Transform {
    ...
    Transform(String n, java.util.function.BiFunction<Integer, Integer, int[]> f) {
      name = n;
      fn = f;
    }

    int[] apply(int dx, int dy) {
      return fn.apply(dx, dy);
    }
  }
  ...
  static Map<String, String> buildBoard(List<BoardSnapshotStone> stones, int boardSize) {
    ...
    if (stones != null)
      for (BoardSnapshotStone s : stones) {
        RC c = gtpToCoord(s.point, boardSize);
        if (c != null) board.put(key(c.row, c.col), s.color);
      }
    return board;
  }
  ...
  static boolean stateMatches(String actual, LocalPatternPointState expected, String perspective) {
    ...
    if (expected == LocalPatternPointState.friendly)
      return actual != null && actual.equals(perspective);
  ...
  static List<RC> neighbors(int row, int col, int boardSize) {
    ...
    for (int[] dd : d) {
      int r = row + dd[0], c = col + dd[1];
      if (r >= 0 && c >= 0 && r < boardSize && c < boardSize) out.add(new RC(r, c));
    }
    return out;
  }

  static int libertyCount(Map<String, String> board, int row, int col, int boardSize) {
    String color = board.get(key(row, col));
    if (color == null) return 0;
    Set<String> seen = new HashSet<>(), libs = new HashSet<>();
    Deque<RC> stack = new ArrayDeque<>();
    stack.push(new RC(row, col));
    while (!stack.isEmpty()) {
      RC cur = stack.pop();
      String ck = key(cur.row, cur.col);
      ...
      for (RC nx : neighbors(cur.row, cur.col, boardSize)) {
        ...
        if (nc == null) libs.add(nk);
        else if (nc.equals(color)) stack.push(nx);
      }
    }
    return libs.size();
  }
  ...
  static boolean regionMatches(ShapePatternCard card, RC anchor, int boardSize) {
    return card.regions.contains("any")
        || card.regions.contains(pointRegion(anchor.row, anchor.col, boardSize));
  }

  static boolean constraintSatisfied(
      LocalPatternConstraint constraint,
      Map<String, String> board,
      RC anchor,
      int boardSize,
      String perspective) {
    if ("anchor-empty".equals(constraint.type))
      return (board.containsKey(key(anchor.row, anchor.col))) == (constraint.value == 0);
    if ("edge-distance".equals(constraint.type)) {
      int edge =
          Math.min(
              Math.min(anchor.row, anchor.col),
              Math.min(boardSize - 1 - anchor.row, boardSize - 1 - anchor.col));
      return edge <= constraint.value;
    }
    String wanted =
        "min-friendly-liberties".equals(constraint.type) ? perspective : opposite(perspective);
    int best = 0;
    for (Map.Entry<String, String> e : board.entrySet()) {
      ...
      if (Math.max(Math.abs(r - anchor.row), Math.abs(c - anchor.col)) <= 2)
        best = Math.max(best, libertyCount(board, r, c, boardSize));
    }
    return best >= constraint.value;
  }

  static LocalPatternMatch matchPatternAt(
      ShapePatternCard card,
      Map<String, String> board,
      String anchorName,
      int boardSize,
      String perspective) {
    ...
    for (Transform transform : TRANSFORMS) {
      ...
      for (LocalPatternPoint point : card.points) {
        ...
        if (row < 0 || col < 0 || row >= boardSize || col >= boardSize) {
          if (requiredPoint)
            counterEvidence.add("required point off board dx=" + point.dx + ",dy=" + point.dy);
          continue;
        }
        String actual = board.get(key(row, col));
        if (stateMatches(actual, point.state, perspective)) {
          matched += requiredPoint ? 1 : 0.5;
          evidence.add(point.state + "@" + point.dx + "," + point.dy);
        } else if (requiredPoint)
          counterEvidence.add("expected " + point.state + "@" + point.dx + "," + point.dy);
      }
      boolean antiHit =
          card.antiPatterns.stream()
              .anyMatch(
                  p -> {
      ...
      boolean constraintsOk =
          card.constraints.stream()
              .allMatch(c -> constraintSatisfied(c, board, anchor, boardSize, perspective));
      if (!constraintsOk) counterEvidence.add("constraint failed");
      double ratio = required > 0 ? matched / required : 0;
      int rawScore =
          (int)
              Math.round(
                  ratio * 20 + Math.min(8, matched) + (constraintsOk ? 4 : 0) - (antiHit ? 8 : 0));
      double minScore = card.minScore != null ? card.minScore : 16;
      if (rawScore < minScore
          || counterEvidence.size() > Math.max(1, (int) (required - matched + 1))) continue;
      ...
      candidate.transform = transform.name;
      candidate.perspective = perspective;
      candidate.matchedPoints = matched;
      candidate.requiredPoints = required;
      evidence.add(0, "anchor=" + anchorName);
      evidence.add(1, "transform=" + transform.name);
      evidence.add(2, "perspective=" + perspective);
  ...
  public static List<LocalPatternMatch> findLocalPatternMatches(
      List<ShapePatternCard> cards, LocalPatternMatcherInput input) {
    Map<String, String> board = buildBoard(input.boardSnapshot, input.boardSize);
    Set<String> anchors = new LinkedHashSet<>(input.anchors);
    if (input.localWindows != null)
      for (LocalWindow w : input.localWindows) if (w.anchor != null) anchors.add(w.anchor);
    anchors.remove(null);
    ...
    for (LocalPatternMatch m : matches) {
      String k = m.card.id + "|" + m.anchor;
