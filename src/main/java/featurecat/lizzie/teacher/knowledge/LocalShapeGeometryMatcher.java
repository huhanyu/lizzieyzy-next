...
/**
 * 对齐 GoAgent knowledge/matchEngine.ts 的 localShapeGeometryMatch（585-648 行）： 以 anchor
 * 为中心的局部形状几何匹配——计算 anchor 的 liberty profile（气数/邻接/组数）， 对 problem 的 initialStones
 * 做旋转/换色变换，比较相对坐标集合与气数 profile，给出匹配分。 problem 来自 training-catalog.json 的 lifeDeathProblems /
 * tesujiProblems。
 */
public final class LocalShapeGeometryMatcher {
  ...
  public static class RelativeStone {
    public int dx, dy;
    public String color;
  }
  ...
  public static class GeometryMatchResult {
    public int score;
    public double ratio;
    public int matched, expected, libertyScore;
  ...
  public static class ProblemEntry {
    ...
    public String teachingRecognition,
        teachingTesujiIdea,
        teachingExplanation,
        teachingMemoryCue,
        teachingFirstFeeling,
        teachingFirstHint;
    public String teachingFailureExplanation;
    public List<String> patternCardIds = new ArrayList<>();

    public String teachingFailureExplanation() {
      return teachingFailureExplanation;
    }
  }

  public static class TrainingMove {
    public String move, explanation, why;
  }

  public static class LocalWindow {
    public String anchor;
    public List<LocalPatternMatcher.BoardSnapshotStone> stones = new ArrayList<>();
  }
  ...
  static final List<RelativeTransform> LOCAL_SHAPE_TRANSFORMS =
      Arrays.asList(
          new RelativeTransform() {
            public int[] apply(int dx, int dy) {
  ...
  static final java.util.regex.Pattern GTP =
      java.util.regex.Pattern.compile("^([A-HJ-T])(\\d{1,2})$");

  static List<RelativeStone> stonesNearAnchor(
      List<LocalPatternMatcher.BoardSnapshotStone> stones, String anchor, int boardSize) {
    ...
    for (LocalPatternMatcher.BoardSnapshotStone s : stones) {
      ...
      seen.add(key);
      RelativeStone r = new RelativeStone();
      r.dx = dx;
      r.dy = dy;
      r.color = s.color;
      out.add(r);
  ...
  static List<LocalPatternMatcher.BoardSnapshotStone> queryStonesForAnchor(
      KnowledgeMatchQuery q, String anchor) {
    if (q.localWindows != null)
      for (LocalWindow w : q.localWindows)
        if (w.anchor.equals(anchor) && !w.stones.isEmpty()) return w.stones;
    return q.boardSnapshot != null ? q.boardSnapshot : new ArrayList<>();
  }

  static String boardKey(int r, int c) {
    return r + "," + c;
  }

  static Map<String, String> localBoard(
      List<LocalPatternMatcher.BoardSnapshotStone> stones, int boardSize) {
    Map<String, String> b = new LinkedHashMap<>();
    for (LocalPatternMatcher.BoardSnapshotStone s : stones) {
      Point p = gtpToPoint(s.point, boardSize);
      if (p != null) b.put(boardKey(p.row, p.col), s.color);
    }
    return b;
  }
  ...
  static int localLibertyCount(Map<String, String> board, List<Point> group, int boardSize) {
    ...
    for (Point s : group)
      for (Point n : neighbors(s.row, s.col, boardSize)) {
        String k = boardKey(n.row, n.col);
        if (!board.containsKey(k)) libs.add(k);
      }
    return libs.size();
  }

  static AnchorLibertyProfile anchorLibertyProfile(
      List<LocalPatternMatcher.BoardSnapshotStone> stones, String anchor, int boardSize) {
    Point ap = gtpToPoint(anchor, boardSize);
    ...
    for (Point n : vns) {
      ...
      if (color == null) {
        profile.adjacent.put("empty", profile.adjacent.get("empty") + 1);
        continue;
      }
  ...
  static String swappedColor(String c) {
    return c.equals("B") ? "W" : "B";
  }
  ...
  static int profileValue(AnchorLibertyProfile p, String color, String field, boolean swap) {
    ...
    if ("minLiberties".equals(field)) {
      Integer v = p.minLiberties.get(sc);
      return v == null ? 0 : v;
    }
  ...
  static int anchorProfileScore(AnchorLibertyProfile qp, AnchorLibertyProfile pp, boolean swap) {
    ...
    for (String color : new String[] {"B", "W"}) {
      score +=
          closeCountScore(qp.adjacent.get(color), profileValue(pp, color, "adjacent", swap), 2, 1);
  ...
  static Integer profileValue2(AnchorLibertyProfile p, String color, boolean swap) {
    return profileValue(p, color, "minLiberties", swap);
  }
  ...
  static List<String> uniqueValidPoints(List<String> moves, int boardSize) {
    ...
    if (moves != null)
      for (String m : moves)
        if (m != null && gtpToPoint(m, boardSize) != null) out.add(m.toUpperCase());
  ...
  public static GeometryMatchResult match(ProblemEntry problem, KnowledgeMatchQuery query) {
    List<String> qAnchors =
        uniqueValidPoints(
            new ArrayList<>() {
              {
                if (query.playedMove != null) add(query.playedMove);
                if (query.candidateMoves != null)
                  addAll(query.candidateMoves.stream().limit(6).toList());
                if (query.principalVariation != null)
                  addAll(query.principalVariation.stream().limit(4).toList());
                if (query.localWindows != null)
                  for (LocalWindow w : query.localWindows) add(w.anchor);
              }
            },
            query.boardSize);
    List<String> pAnchors =
        uniqueValidPoints(
            problem.correctMoves.stream().limit(2).map(m -> m.move).toList(), query.boardSize);
    ...
    for (String qa : qAnchors) {
      ...
      for (String pa : pAnchors) {
        AnchorLibertyProfile pProfile =
            anchorLibertyProfile(problem.initialStones, pa, query.boardSize);
        ...
        for (RelativeTransform t : LOCAL_SHAPE_TRANSFORMS) {
          for (boolean swap : new boolean[] {false, true}) {
            ...
            cand.libertyScore = libertyScore;
            cand.transform = t.name();
            cand.colorMode = swap ? "color-swapped" : "same-color";
            cand.queryAnchor = qa;
            cand.problemAnchor = pa;
            if (best == null
                || cand.score > best.score
                || (cand.score == best.score && cand.ratio > best.ratio)) best = cand;
          }
        }
      }
    }
  ...
  public static GeometryMatchResult matchProblems(
      List<ProblemEntry> problems, KnowledgeMatchQuery query) {
    GeometryMatchResult best = null;
