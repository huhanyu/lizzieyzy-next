...
/** 对齐 GoAgent knowledge/motifRecognizer.ts： 综合 elite 卡、定式识别、知识匹配、启发式，识别当前局面的棋形/motif（教学要点）。 */
public final class MotifRecognizer {

  private MotifRecognizer() {}

  public enum MotifConfidence {
    strong,
    medium,
    weak
  }

  public enum MotifPhase {
    opening,
    middlegame,
    endgame,
    any
  ...
  public static class ExpectedNextMove {
    public String move, label, condition;
  }

  public static class MotifRecognizerQuery {
    public java.util.List<MatchEngine.KnowledgeMatch> knowledgeMatches =
  ...
  static String stringifySignals(Object... parts) {
    ...
    for (Object part : parts) {
      ...
      if (part instanceof Object[]) {
        for (Object o : (Object[]) part) sb.append(String.valueOf(o).toLowerCase()).append(" | ");
  ...
  static String[] tokenHit(String signalText, String[] tokens) {
    ...
    for (String t : tokens)
      if (t != null && !t.isEmpty() && signalText.contains(t.toLowerCase())) hits.add(t);
    return hits.toArray(new String[0]);
  }

  static class ScoreWithReasons {
    double score;
    List<String> reasons = new ArrayList<>();
  }

  static ScoreWithReasons scoreEliteCard(
      JsonKnowledgeLoader.ElitePatternCard card,
      MotifRecognizerQuery query,
      String signalText,
      MotifRegion region,
      MotifPhase phase,
      int localDistance,
    ...
    if (phaseHit) {
      score += 2;
      reasons.add("phase=" + phase);
    }
    boolean regionHit = contains(card.regions, region.name()) || contains(card.regions, "any");
    if (regionHit) {
      score += 2;
      reasons.add("region=" + region);
    }
    if (query.userLevel == null
        || contains(card.levels, query.userLevel)
        || contains(card.levels, "any")) score += 1;
    String[] tagHits = tokenHit(signalText, concat(card.tags, card.aliases));
    if (tagHits.length > 0) {
      score += Math.min(7, tagHits.length * 2);
      reasons.add(
          "keyword hits: "
              + String.join(", ", Arrays.copyOf(tagHits, Math.min(4, tagHits.length))));
    }
    String[] trigHits = tokenHit(signalText, card.triggerSignals);
    if (trigHits.length > 0) {
      score += Math.min(8, trigHits.length * 2.5);
      reasons.add(
          "trigger signals: "
              + String.join(", ", Arrays.copyOf(trigHits, Math.min(4, trigHits.length))));
    }
    String[] kataHits = tokenHit(signalText, card.katagoSignals);
    if (kataHits.length > 0) {
      score += Math.min(6, kataHits.length * 2);
      reasons.add(
          "KataGo signals: "
              + String.join(", ", Arrays.copyOf(kataHits, Math.min(4, kataHits.length))));
    }
    String[] negHits = tokenHit(signalText, card.negativeSignals);
    if (negHits.length > 0) score -= negHits.length * 3;
    if (loss >= 7 || scoreLoss >= 4 || "blunder".equals(judgement)) {
      score += 3;
      reasons.add("large-loss review point");
    } else if (loss >= 2.5 || scoreLoss >= 1.5 || "mistake".equals(judgement)) {
      score += 2;
      reasons.add("medium-loss review point");
    }
    if (localDistance <= 2
        && (card.category.equals("tesuji")
            || card.category.equals("life-death")
            || card.category.equals("shapes"))) {
      score += 2;
      reasons.add("actual/best move are local alternatives");
    }
    if (localDistance >= 6
        && (card.category.equals("strategy")
            || card.category.equals("opening")
            || card.category.equals("endgame"))) {
      score += 2;
      reasons.add("actual/best move indicate whole-board direction");
    }
    if (spread < 1.5 && card.patternType.contains("style")) score += 3;
    ScoreWithReasons r = new ScoreWithReasons();
    r.score = score;
    r.reasons = reasons;
    return r;
  }
  ...
  static boolean contains(String[] arr, String v) {
    ...
    for (String a : arr) if (a != null && a.equals(v)) return true;
    return false;
  }

  static String[] concat(String[]... arrs) {
    List<String> out = new ArrayList<>();
    for (String[] a : arrs) if (a != null) for (String s : a) out.add(s);
    return out.toArray(new String[0]);
  }

  static RecognizedTeachingMotif motifFromCard(
      JsonKnowledgeLoader.ElitePatternCard card,
      double score,
      List<String> reasons,
      MotifRegion region,
    ...
    m.recognition = card.recognition;
    m.wrongThinking = card.wrongThinking;
    m.correctThinking = card.correctThinking;
    m.drillPrompt = card.drillPrompt;
    m.source = "elite-card";
    m.sourceRefs = card.sourceRefs;
    m.sourceQuality = card.sourceQuality;
    m.relatedMoves = relatedMoves;
    m.tags = card.tags;
    return m;
  }

  static List<RecognizedTeachingMotif> matchKnowledgeMotifs(
      java.util.List<MatchEngine.KnowledgeMatch> matches, String[] relatedMoves) {
    ...
    for (MatchEngine.KnowledgeMatch match : matches) {
      ...
      if (match.teachingPayload != null && match.teachingPayload.recognition != null)
        evidence.add(match.teachingPayload.recognition);
      ...
      m.confidence =
          "strong".equals(match.confidence) ? MotifConfidence.strong : MotifConfidence.medium;
      m.score = Math.round((match.score + ("strong".equals(match.confidence) ? 5 : 2)) * 10) / 10.0;
      m.evidence = evidence.toArray(new String[0]);
      m.whyMatched =
          (evidence.size() > 3
              ? String.join("；", evidence.subList(0, 3))
              : String.join("；", evidence));
      if (m.whyMatched.isEmpty()) m.whyMatched = "知识匹配 " + match.title;
      m.recognition =
          match.teachingPayload != null && match.teachingPayload.recognition != null
              ? match.teachingPayload.recognition
              : match.title;
  ...
  static List<RecognizedTeachingMotif> heuristicMotifs(
      MotifRecognizerQuery query,
      MotifPhase phase,
      MotifRegion region,
      String actual,
    ...
    if ((loss >= 2.5
            || scoreLoss >= 1.5
            || "mistake".equals(judgement)
            || "blunder".equals(judgement))
        && localDistance >= 6) {
      results.add(
          makeHeuristic(
              "heuristic-urgent-vs-big",
              "急所与大场的优先级",
              "urgent_vs_big",
              "strategy",
              loss >= 7 || scoreLoss >= 4 ? MotifConfidence.strong : MotifConfidence.medium,
              11 + Math.min(5, loss / 2),
              region,
              phase,
              relatedMoves,
    ...
    if ((loss >= 3 || scoreLoss >= 2) && localDistance <= 2 && phase != MotifPhase.opening) {
      results.add(
          makeHeuristic(
              "heuristic-local-shape-loss",
              "局部棋形/读秒下的细节亏损",
              "shape_inefficiency",
              "shapes",
              loss >= 7 ? MotifConfidence.strong : MotifConfidence.medium,
              10 + Math.min(4, loss / 2),
              region,
              phase,
              relatedMoves,
              new String[] {
                "实战手和首选手是近距离局部选择", "localDistance=" + localDistance, "judgement=" + judgement
              },
    ...
    if (phase == MotifPhase.endgame && (scoreLoss >= 1 || loss >= 2.5)) {
      results.add(
          makeHeuristic(
              "heuristic-endgame-sente",
              "官子先后手与目差",
              "endgame_sente",
              "endgame",
              scoreLoss >= 2 ? MotifConfidence.strong : MotifConfidence.medium,
              10 + Math.min(5, scoreLoss * 2),
              region,
              phase,
              relatedMoves,
    ...
    if (phase == MotifPhase.middlegame
        && localDistance >= 4
        && (query.principalVariation != null && query.principalVariation.length >= 3)) {
      results.add(
          makeHeuristic(
              "heuristic-attack-direction",
              "攻击方向与借力",
              "attack_direction",
              "strategy",
              loss >= 4 ? MotifConfidence.medium : MotifConfidence.weak,
              8 + Math.min(4, loss / 2),
              region,
              phase,
              relatedMoves,
  ...
  static RecognizedTeachingMotif makeHeuristic(
      String id,
      String title,
      String motifType,
      String category,
      MotifConfidence conf,
      double score,
      MotifRegion region,
      MotifPhase phase,
      String[] relatedMoves,
    ...
    m.id = id;
    m.title = title;
    m.motifType = motifType;
    m.category = category;
    m.phase = new MotifPhase[] {phase};
    m.region = region;
    m.confidence = conf;
    m.score = Math.round(score * 10) / 10.0;
    m.evidence = evidence;
    m.whyMatched = String.join("；", evidence);
    m.recognition = recognition;
    m.wrongThinking = wrong;
    m.correctThinking = correct;
    m.drillPrompt = drill;
    m.source = "heuristic";
    m.relatedMoves = relatedMoves;
    m.tags = tags;
    m.sourceRefs = new String[] {"heuristic"};
    return m;
  }
  ...
  static RecognizedTeachingMotif[] motifsFromJoseki(JosekiRecognizer.RecognizedJosekiPattern p) {
    ...
    m.id = "joseki-" + p.id;
    m.title = p.name;
    m.motifType = "joseki:" + p.family;
    m.category = "joseki";
    m.phase = new MotifPhase[] {MotifPhase.opening};
    m.region = MotifRegion.corner;
    m.confidence =
        p.confidence == JosekiRecognizer.JosekiConfidence.strong
            ? MotifConfidence.strong
            : p.confidence == JosekiRecognizer.JosekiConfidence.medium
                ? MotifConfidence.medium
                : MotifConfidence.weak;
    m.score = p.score + 2;
    m.evidence = p.evidence;
    m.whyMatched = String.join("；", Arrays.copyOf(p.evidence, Math.min(4, p.evidence.length)));
    m.recognition = p.recognition;
    m.wrongThinking = p.wrongThinking;
    m.correctThinking = p.correctThinking;
    m.drillPrompt = p.drillPrompt;
    m.source = "joseki-card";
    m.sourceRefs = p.sourceRefs;
    m.sourceQuality = p.sourceQuality;
    m.variationCount = p.variationCount;
    m.josekiFamily = p.family;
    m.relatedMoves =
        p.commonNextMoves.stream()
            .map(mv -> mv.gtpMove != null ? mv.gtpMove : mv.relativeMove)
  ...
  public static List<RecognizedTeachingMotif> recognizeTeachingMotifs(MotifRecognizerQuery query) {
    ...
    String bestMove =
        normalizeMove(
            query.candidateMoves != null && query.candidateMoves.length > 0
                ? query.candidateMoves[0]
                : null);
    String actualMove =
        normalizeMove(
            query.playedMove != null && query.playedMove.length > 0 ? query.playedMove[0] : null);
    ...
    String[] relatedMoves =
        concat(
            new String[] {actualMove, bestMove},
            query.principalVariation != null
                ? Arrays.copyOf(
                    query.principalVariation, Math.min(6, query.principalVariation.length))
                : new String[0]);
    ...
    implied.add(phaseSignal);
    implied.add(regionSignal);
    ...
    implied.add(
        scoreLoss >= 4
            ? "high_score_loss"
            : scoreLoss >= 1.5 ? "medium_score_loss" : "small_score_loss");
    ...
    for (JsonKnowledgeLoader.ElitePatternCard card : JsonKnowledgeLoader.loadEliteCards()) {
      MotifRecognizer.ScoreWithReasons scr =
          scoreEliteCard(
              card,
              query,
              signalText,
              ...
              spread);
      if (scr.score >= 8)
        cardMotifs.add(motifFromCard(card, scr.score, scr.reasons, region, relatedMoves));
    }
    ...
    jq.boardSize = query.boardSize;
    jq.moveNumber = query.moveNumber;
    jq.recentMoves = query.recentMoves;
    jq.candidateMoves = query.candidateMoves;
    jq.principalVariation = query.principalVariation;
    jq.actualMove = actualMove;
    jq.bestMove = bestMove;
    jq.text = query.text;
    jq.maxResults = 4;
    ...
    List<RecognizedTeachingMotif> heuristics =
        heuristicMotifs(
            query,
            phase,
            region,
    ...
    all.addAll(josekiMotifs);
    all.addAll(cardMotifs);
    all.addAll(heuristics);
  ...
  public static String formatRecognizedMotifsForPrompt(List<RecognizedTeachingMotif> motifs) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (m.sourceRefs != null && m.sourceRefs.length > 0)
        sb.append(
            "来源标记："
                + String.join(", ", m.sourceRefs)
                + "；sourceQuality="
                + (m.sourceQuality != null ? m.sourceQuality : "unknown")
                + "\n");
      ...
      if (m.expectedNextMoves != null && !m.expectedNextMoves.isEmpty()) {
        ...
        for (int k = 0; k < Math.min(4, m.expectedNextMoves.size()); k++) {
          ...
          en.append(mv.move)
              .append(" ")
              .append(mv.label)
              .append(mv.condition != null ? "(" + mv.condition + ")" : "")
              .append("；");
        }
        sb.append("常见下一手：" + en + "\n");
      }
