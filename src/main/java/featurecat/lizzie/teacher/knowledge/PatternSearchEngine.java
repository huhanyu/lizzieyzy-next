...
/**
 * 对齐 GoAgent knowledge/patterns.ts（329 行）： loadKnowledgePatternCards + searchKnowledgePatterns +
 * formatPatternForPrompt + patternToSearchText。 基于 KnowledgePatternCard.triggers
 * 的加权打分（level/phase/region/moveNumber/lossScore/judgement/
...
public final class PatternSearchEngine {
  ...
  public static class Variation {
    public String name, mainLine, whenToChoose, warning;
  }

  public static class Teaching {
    public String recognition, correctIdea, memoryCue, commonMistake, drill;
  }

  public static class Triggers {
    public List<String> moveFeatures = new ArrayList<>(),
        candidateFeatures = new ArrayList<>(),
        pvFeatures = new ArrayList<>(),
        contextTags = new ArrayList<>(),
        judgements = new ArrayList<>();
    public Integer minMoveNumber, maxMoveNumber;
    public Double minLossScore;
  }

  public static class Shape {
    public String anchor;
    public List<String> canonicalMoves = new ArrayList<>(), gtpExamples = new ArrayList<>();
  ...
  public static class PatternSearchMatch {
    public KnowledgePatternCard card;
    public double score;
    public PatternConfidence confidence;
  ...
  public static List<KnowledgePatternCard> loadKnowledgePatternCards() {
    ...
    try (InputStream in =
        PatternSearchEngine.class.getResourceAsStream("/knowledge/pattern-cards.json")) {
      if (in == null) {
        cachedRoot = "resources";
      ...
      if (root instanceof List) {
        ...
        for (Object item : (List<?>) root)
          if (item instanceof Map) out.add(parseCard((Map<?, ?>) item));
        cachedRoot = "resources";
        cachedCards = out;
        return out;
      }
  ...
  private static KnowledgePatternCard parseCard(Map<?, ?> m) {
    ...
    c.id = str(m.get("id"));
    c.title = str(m.get("title"));
    c.category = str(m.get("category"));
    c.patternType = str(m.get("patternType"));
    c.phase = strList(m.get("phase"));
    c.levels = strList(m.get("levels"));
    c.regions = strList(m.get("regions"));
    c.tags = strList(m.get("tags"));
    c.aliases = strList(m.get("aliases"));
    c.boardSignals = strList(m.get("boardSignals"));
    Object tr = m.get("triggers");
    if (tr instanceof Map) {
      Map<?, ?> tm = (Map<?, ?>) tr;
      c.triggers.moveFeatures = strList(tm.get("moveFeatures"));
      c.triggers.candidateFeatures = strList(tm.get("candidateFeatures"));
      c.triggers.pvFeatures = strList(tm.get("pvFeatures"));
      c.triggers.contextTags = strList(tm.get("contextTags"));
      c.triggers.judgements = strList(tm.get("judgements"));
      c.triggers.minMoveNumber =
          tm.get("minMoveNumber") instanceof Number
              ? ((Number) tm.get("minMoveNumber")).intValue()
              : null;
      c.triggers.maxMoveNumber =
          tm.get("maxMoveNumber") instanceof Number
              ? ((Number) tm.get("maxMoveNumber")).intValue()
              : null;
      c.triggers.minLossScore =
          tm.get("minLossScore") instanceof Number
              ? ((Number) tm.get("minLossScore")).doubleValue()
              : null;
    }
    Object sh = m.get("shape");
    if (sh instanceof Map) {
      Map<?, ?> sm = (Map<?, ?>) sh;
      c.shape.anchor = str(sm.get("anchor"));
      c.shape.symmetry = str(sm.get("symmetry"));
      c.shape.canonicalMoves = strList(sm.get("canonicalMoves"));
      c.shape.gtpExamples = strList(sm.get("gtpExamples"));
    }
    ...
    if (vars instanceof List)
      for (Object v : (List<?>) vars)
        if (v instanceof Map) {
          ...
          var.name = str(vm.get("name"));
          var.mainLine = str(vm.get("mainLine"));
          var.whenToChoose = str(vm.get("whenToChoose"));
          var.warning = str(vm.get("warning"));
          c.variations.add(var);
        }
    Object t = m.get("teaching");
    if (t instanceof Map) {
      Map<?, ?> tm = (Map<?, ?>) t;
      c.teaching.recognition = str(tm.get("recognition"));
      c.teaching.correctIdea = str(tm.get("correctIdea"));
      c.teaching.memoryCue = str(tm.get("memoryCue"));
      c.teaching.commonMistake = str(tm.get("commonMistake"));
      c.teaching.drill = str(tm.get("drill"));
    }
    return c;
  }
  ...
  static List<String> normalizeTokens(List<String> values) {
    ...
    for (String v : values)
      if (v != null)
        for (String tok : v.toLowerCase().split("[，。！？、；：,.!?;:()\\[\\]【】\\s/]+")) {
          tok = tok.trim();
  ...
  static Pt gtpToPoint(String point, int boardSize) {
    ...
    var m =
        java.util.regex.Pattern.compile("^([A-HJ-Z])(\\d{1,2})$")
  ...
  static Set<String> moveFeaturesFromGtp(List<String> points, int boardSize) {
    ...
    if (points != null)
      for (String p : points) {
        Pt parsed = gtpToPoint(p, boardSize);
        if (parsed != null) addPointFeatures(features, parsed.row, parsed.col, boardSize);
      }
    return features;
  }
  ...
  static double textScore(
      KnowledgePatternCard card, PatternSearchContext ctx, List<String> reasons) {
    List<String> queryVals = new ArrayList<>();
    if (ctx.text != null) queryVals.add(ctx.text);
    queryVals.addAll(ctx.contextTags);
    ...
    cardVals.addAll(card.tags);
    cardVals.addAll(card.aliases);
    cardVals.addAll(card.boardSignals);
    cardVals.addAll(
        Arrays.asList(
            card.teaching.recognition, card.teaching.correctIdea, card.teaching.memoryCue));
    List<String> cardTokens = normalizeTokens(cardVals);
    double score = 0;
    for (String token : queryTokens) {
      if (cardTokens.stream().anyMatch(cand -> cand.contains(token) || token.contains(cand))) {
        score += 4;
  ...
  static PatternConfidence confidenceFromScore(double score) {
    if (score >= 18) return PatternConfidence.high;
    if (score >= 10) return PatternConfidence.medium;
    return PatternConfidence.low;
  }

  public static List<PatternSearchMatch> searchKnowledgePatterns(
      List<KnowledgePatternCard> cards, PatternSearchContext ctx) {
    Set<String> contextTags = new LinkedHashSet<>(normalizeTokens(ctx.contextTags));
    Set<String> recentFeatures =
        moveFeaturesFromRecord(
            ctx.recentMoves.size() > 8
                ? ctx.recentMoves.subList(ctx.recentMoves.size() - 8, ctx.recentMoves.size())
                : ctx.recentMoves,
            ctx.boardSize);
    Set<String> playedFeatures =
        moveFeaturesFromGtp(
            ctx.playedMove != null ? Collections.singletonList(ctx.playedMove) : new ArrayList<>(),
            ctx.boardSize);
    ...
    Set<String> moveFeatures = new LinkedHashSet<>(recentFeatures);
    moveFeatures.addAll(playedFeatures);

    List<PatternSearchMatch> matches = new ArrayList<>();
    for (KnowledgePatternCard card : cards) {
      double score = 0;
      List<String> reasons = new ArrayList<>();
      if (card.levels.contains(ctx.userLevel)) {
        score += 2;
        reasons.add("level:" + ctx.userLevel);
      }
      if (card.phase.contains(ctx.phase)) {
        score += 4;
        reasons.add("phase:" + ctx.phase);
      }
      if (card.regions.contains(ctx.region)) {
        score += 5;
        reasons.add("region:" + ctx.region);
      }
      int minMove = card.triggers.minMoveNumber != null ? card.triggers.minMoveNumber : 0;
      int maxMove =
          card.triggers.maxMoveNumber != null ? card.triggers.maxMoveNumber : Integer.MAX_VALUE;
      if (ctx.moveNumber >= minMove && ctx.moveNumber <= maxMove) {
        if (card.triggers.minMoveNumber != null || card.triggers.maxMoveNumber != null) score += 2;
      }
      double minLoss =
          card.triggers.minLossScore != null
              ? card.triggers.minLossScore
              : Double.POSITIVE_INFINITY;
      if ((ctx.lossScore != null ? ctx.lossScore : 0) >= minLoss) {
        score += 3;
        reasons.add("loss>=" + card.triggers.minLossScore);
      }
      if (ctx.judgement != null
          && !ctx.judgement.isEmpty()
          && card.triggers.judgements.contains(ctx.judgement)) {
        score += 2;
        reasons.add("judgement:" + ctx.judgement);
      }
      for (String tag : card.triggers.contextTags) {
        String normalized = tag.toLowerCase();
        if (contextTags.contains(normalized)
            || contextTags.stream()
      ...
      score +=
          overlapScore(card.triggers.candidateFeatures, candidateFeatures, 6, "candidate", reasons);
      score += overlapScore(card.triggers.pvFeatures, pvFeatures, 3, "pv", reasons);
      score += textScore(card, ctx, reasons);
      if ("joseki".equals(card.category)
          && "opening".equals(ctx.phase)
          && "corner".equals(ctx.region)) {
        score += 3;
        reasons.add("joseki-opening-corner");
      }
      if ("life_death".equals(card.category)
          && (ctx.lossScore != null ? ctx.lossScore : 0) >= 2
          && (ctx.region.equals("corner") || ctx.region.equals("side"))) {
        score += 3;
        reasons.add("life-death-risk");
      }
      if (score >= 8) {
        PatternSearchMatch m = new PatternSearchMatch();
        m.card = card;
        m.score = score;
        m.confidence = confidenceFromScore(score);
  ...
  public static String formatPatternForPrompt(PatternSearchMatch match) {
    ...
    for (Variation v : card.variations.subList(0, Math.min(3, card.variations.size()))) {
      ...
      if (v.warning != null && !v.warning.isEmpty())
        sb.append("   注意: ").append(v.warning).append("\n");
    }
    return String.join(
        "\n",
  ...
  public static String patternToSearchText(KnowledgePatternCard card) {
    ...
    parts.addAll(card.tags);
    parts.addAll(card.aliases);
    parts.addAll(card.boardSignals);
    parts.addAll(card.shape.canonicalMoves);
    parts.addAll(
        Arrays.asList(
            ...
            card.teaching.drill));
    for (Variation v : card.variations)
      parts.addAll(
          Arrays.asList(v.name, v.mainLine, v.whenToChoose, v.warning != null ? v.warning : ""));
