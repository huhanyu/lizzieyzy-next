...
public final class SearchLocal {
  ...
  public static class KnowledgeSearchQuery {
    public String text;
    public List<String> errorTypes = new ArrayList<>(), tags = new ArrayList<>();
    public String phase;
    public int limit = 4;
  }
  ...
  public static List<KnowledgeCard> loadKnowledgeCards() {
      ...
      if (root instanceof List)
        for (Object item : (List<?>) root)
          if (item instanceof Map) {
            ...
            c.kind = str(m.get("kind"));
            c.summary = str(m.get("summary"));
            c.coachShort = str(m.get("coachShort"));
            c.coachLong = str(m.get("coachLong"));
            c.drill = str(m.get("drill"));
            c.phase = strList(m.get("phase"));
            c.errorTypes = strList(m.get("errorTypes"));
            c.tags = strList(m.get("tags"));
            c.katagoSignals = strList(m.get("katagoSignals"));
            c.boardSignals = strList(m.get("boardSignals"));
            c.related = strList(m.get("related"));
            out.add(c);
          }
    } catch (Exception e) {
      /* ignore */
    ...
    try {
      java.util.List<PatternSearchEngine.KnowledgePatternCard> pCards =
          PatternSearchEngine.loadKnowledgePatternCards();
      for (PatternSearchEngine.KnowledgePatternCard pc : pCards)
        out.add(patternToKnowledgeCard(pc));
    } catch (Exception e) {
      /* ignore */
    }
    cachedP0 = out;
  ...
  static KnowledgeCard patternToKnowledgeCard(PatternSearchEngine.KnowledgePatternCard card) {
    ...
    c.title = card.title;
    c.kind =
        "joseki".equals(card.category)
            ? "joseki"
            : "life_death".equals(card.category) ? "life_death" : "shape";
    c.phase = card.phase != null ? new ArrayList<>(card.phase) : new ArrayList<>();
    if ("joseki".equals(card.category))
      c.errorTypes = new ArrayList<>(java.util.Arrays.asList("direction"));
    else if ("life_death".equals(card.category))
      c.errorTypes = new ArrayList<>(java.util.Arrays.asList("life-death", "reading"));
    ...
    c.katagoSignals =
        card.triggers != null && card.triggers.candidateFeatures != null
            ? new ArrayList<>(card.triggers.candidateFeatures)
            : new ArrayList<>();
    java.util.Set<String> board = new java.util.LinkedHashSet<>();
    if (card.boardSignals != null) board.addAll(card.boardSignals);
    if (card.shape != null && card.shape.canonicalMoves != null)
      board.addAll(card.shape.canonicalMoves);
  ...
  static List<String> strList(Object o) {
    ...
    if (o instanceof List) for (Object x : (List<?>) o) if (x != null) l.add(String.valueOf(x));
    return l;
  }
  ...
  static String cardText(Object card) {
    if (card instanceof JosekiLineEntry e)
      return String.join(
          " ",
          Arrays.asList(
              e.title,
              e.family,
              e.katagoEraJudgement,
              String.join(" ", e.tags),
              String.join(" ", e.relativeSequence)));
    if (card instanceof ShapePatternCard c)
      return String.join(
          " ",
          Arrays.asList(
              c.title,
              ...
              c.category,
              String.join(" ", c.tags),
              c.teaching.recognition,
              c.teaching.correctThinking));
    if (card instanceof KnowledgeCard k)
      return String.join(
          " ",
          Arrays.asList(
              k.title,
              ...
              k.coachLong,
              k.drill,
              String.join(" ", k.errorTypes),
              String.join(" ", k.tags),
              String.join(" ", k.katagoSignals),
              String.join(" ", k.boardSignals)));
    return "";
  }
  ...
  public static List<KnowledgeSearchResult> search(List<Object> cards, KnowledgeSearchQuery query) {
    ...
    for (Object card : cards) {
      ...
      if (query.phase != null && cardPhases(card).contains(query.phase)) {
        score += 2;
        reasons.add("phase:" + query.phase);
      }
      for (String err : cardErrors(card))
        if (wantedErrors.contains(err.toLowerCase())) {
          score += 5;
          reasons.add("error:" + err);
        }
      for (String tag : cardTags(card))
        if (wantedTags.contains(tag.toLowerCase())) {
          score += 3;
          reasons.add("tag:" + tag);
        }
      if (score == 0 && terms.isEmpty() && wantedErrors.isEmpty() && wantedTags.isEmpty()) {
        score = 1;
        reasons.add("default");
      }
      if (score > 0) {
    ...
    results.sort(
        (a, b) ->
            Double.compare(b.score, a.score) != 0
                ...
                : cardTitle(a.card).compareTo(cardTitle(b.card)));
  ...
  public static String formatKnowledgeCardsForPrompt(List<KnowledgeSearchResult> results) {
    ...
    for (KnowledgeSearchResult r : results.subList(0, Math.min(4, results.size()))) {
      Object card = r.card;
      String title =
          card instanceof JosekiLineEntry e
              ? e.title
              : card instanceof ShapePatternCard c
                  ? c.title
                  : card instanceof KnowledgeCard k ? k.title : "?";
      String kind =
          card instanceof JosekiLineEntry
              ? "joseki"
              : card instanceof ShapePatternCard c
                  ? c.category
                  : card instanceof KnowledgeCard k ? k.kind : "?";
      List<String> errors = cardErrors(card);
      String summary, coachShort, drill;
      if (card instanceof KnowledgeCard k) {
        summary = k.summary;
        coachShort = k.coachShort;
        drill = k.drill;
      } else if (card instanceof ShapePatternCard c) {
        summary = c.teaching.recognition;
        coachShort = c.teaching.correctThinking;
        drill = c.teaching.drillPrompt;
      } else if (card instanceof JosekiLineEntry e) {
        summary = e.katagoEraJudgement;
        coachShort = String.join(" ", e.decisionRules);
        drill = "按方向与先手判断分支选择";
      } else {
        summary = "";
        coachShort = "";
