...
/**
 * 对齐 GoAgent knowledge/shapeRecognitionEngine.ts：综合局部模式匹配（LocalPatternMatcher）+ KataGo
 * 形状特征（KatagoShapeFeatures）输出识别棋形。 数据加载走
 * JsonKnowledgeLoader（resources/knowledge/shape-pattern-cards-v1.json 等）。
 */
public final class ShapeRecognitionEngine {

  private ShapeRecognitionEngine() {}

  public enum ShapeRecognitionConfidence {
    strong,
    medium,
    weak
  }
  ...
  public static class RecognizedShape {
    ...
    public List<String> evidence = new ArrayList<>(),
        counterEvidence = new ArrayList<>(),
        relatedMoves = new ArrayList<>();
  ...
  static ShapeRecognitionConfidence featureConfidence(double score) {
    return score >= 22
        ? ShapeRecognitionConfidence.strong
        : score >= 15 ? ShapeRecognitionConfidence.medium : ShapeRecognitionConfidence.weak;
  }
  ...
  static RecognizedShape fromLocalMatch(LocalPatternMatcher.LocalPatternMatch m) {
    ...
    s.title = m.card.title;
    s.shapeType = m.card.shapeType;
    s.category = m.card.category;
    s.confidence = m.confidence;
    s.score = m.score;
    s.evidence = m.evidence;
    s.counterEvidence = m.counterEvidence;
    s.safeWording = safeWording(m.confidence, m.counterEvidence);
    s.relatedMoves = new ArrayList<>(Collections.singletonList(m.anchor));
    s.recognition = m.card.teaching.recognition;
    s.wrongThinking = m.card.teaching.wrongThinking;
    s.correctThinking = m.card.teaching.correctThinking;
    s.drillPrompt = m.card.teaching.drillPrompt;
    s.sourceRefs = m.card.sourceRefs;
    s.sourceQuality = m.card.sourceQuality;
    return s;
  }
  ...
  static List<RecognizedShape> uniqueShapes(List<RecognizedShape> shapes) {
    ...
    for (RecognizedShape shape : shapes) {
      String key =
          shape.shapeType + ":" + (shape.relatedMoves.isEmpty() ? "" : shape.relatedMoves.get(0));
  ...
  public static class ShapeRecognitionInput {
    ...
    public List<String> recentMoves = new ArrayList<>(),
  ...
  public static List<RecognizedShape> recognizeShapes(
      ShapeRecognitionInput input, List<LocalPatternMatcher.ShapePatternCard> cards) {
    ...
    anchors.addAll(
        input.principalVariation.subList(0, Math.min(6, input.principalVariation.size())));

    LocalPatternMatcher.LocalPatternMatcherInput lp =
        new LocalPatternMatcher.LocalPatternMatcherInput();
    lp.boardSize = input.boardSize;
    lp.boardSnapshot = input.boardSnapshot;
    lp.localWindows = input.localWindows;
    lp.anchors = anchors;
    lp.playerColor = input.playerColor;
    lp.phase = phase(input.moveNumber, input.totalMoves);
    List<RecognizedShape> localMatches = new ArrayList<>();
    for (LocalPatternMatcher.LocalPatternMatch m :
        LocalPatternMatcher.findLocalPatternMatches(cards, lp)) localMatches.add(fromLocalMatch(m));

    List<RecognizedShape> featureShapes = new ArrayList<>();
    KatagoShapeFeatures.KataGoShapeFeatureInput kf =
        new KatagoShapeFeatures.KataGoShapeFeatureInput();
    kf.boardSize = input.boardSize;
    kf.moveNumber = input.moveNumber;
    kf.totalMoves = input.totalMoves;
    kf.playedMove = input.playedMove;
    kf.candidateMoves = input.candidateMoves;
    kf.principalVariation = input.principalVariation;
    if (input.lossScore != null) {
      try {
        kf.lossScore = Double.parseDouble(input.lossScore);
      } catch (Exception e) {
      }
    }
    ...
    for (KatagoShapeFeatures.KataGoShapeFeature f : KatagoShapeFeatures.extract(kf)) {
      RecognizedShape s = new RecognizedShape();
      s.id = "katago-feature:" + f.id;
      s.title = f.recognition.length() > 28 ? f.recognition.substring(0, 28) : f.recognition;
      s.shapeType = f.shapeType;
      s.category = "katago-shape-feature";
      s.confidence = f.confidence;
      s.score = f.score;
      s.evidence = f.evidence;
      s.counterEvidence = f.counterEvidence;
      s.safeWording = safeWording(f.confidence, f.counterEvidence);
      s.relatedMoves = f.relatedMoves;
      s.recognition = f.recognition;
      s.wrongThinking = f.wrongThinking;
      s.correctThinking = f.correctThinking;
      s.drillPrompt = f.drillPrompt;
      s.sourceRefs =
          new ArrayList<>(Arrays.asList("katago-analysis-engine-docs", "goagent-curated-original"));
    ...
    return uniqueShapes(
            new ArrayList<>() {
              {
                addAll(localMatches);
                addAll(featureShapes);
              }
            })
        .subList(
            0,
            Math.min(
                input.maxResults != null ? input.maxResults : 6,
                localMatches.size() + featureShapes.size()));
  }

  /** 对齐 TS recognizedShapesToKnowledgePackets：过滤"不能主讲"，输出 KnowledgePacket（Map 形式） */
  public static List<java.util.Map<String, Object>> recognizedShapesToKnowledgePackets(
      List<RecognizedShape> shapes) {
    ...
    for (RecognizedShape shape : shapes) {
      ...
      tags.add(shape.shapeType);
      tags.add(shape.confidence);
      tags.add(shape.safeWording);
      if (shape.sourceRefs != null)
        tags.addAll(shape.sourceRefs.subList(0, Math.min(2, shape.sourceRefs.size())));
      p.put("tags", tags);
      ...
      if (!shape.counterEvidence.isEmpty())
        body.add("反证/降置信: " + String.join("；", shape.counterEvidence));
      body.add("常见误区: " + shape.wrongThinking);
      body.add("正确思路: " + shape.correctThinking);
      body.add("小练习: " + shape.drillPrompt);
      body.add(
          "sourceRefs: " + (shape.sourceRefs != null ? String.join(", ", shape.sourceRefs) : ""));
      p.put("selectedBody", String.join("\n", body));
      p.put(
          "score",
          shape.score
              + ("strong".equals(shape.confidence)
                  ? 10
                  : "medium".equals(shape.confidence) ? 5 : 0));
      packets.add(p);
    }
    return packets;
  }
  ...
  public static String formatShapeRecognitionForPrompt(List<RecognizedShape> shapes) {
    ...
    for (RecognizedShape s : shapes.subList(0, Math.min(6, shapes.size()))) {
      ...
      if (!s.counterEvidence.isEmpty())
        sb.append("反证：").append(String.join("；", s.counterEvidence)).append("\n");
