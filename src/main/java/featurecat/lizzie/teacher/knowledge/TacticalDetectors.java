...
/**
 * 对齐 GoAgent knowledge/tacticalDetectors.ts：从棋盘状态检测战术信号 （气不足 / 切断点 / 眼形风险 / 先手逼迫）。需要
 * BoardGroup/BoardState，这里用 lizzieyzy Board 自算棋块与气。
 */
public final class TacticalDetectors {
  ...
  public static class BoardGroup {
    ...
    public List<String> liberties = new ArrayList<>();
  }
  ...
  static String name(int x, int y, int size) {
    return GTP.substring(x, x + 1) + (size - y);
  }
  ...
  static String confidenceFromCount(int count) {
  ...
  public static List<TacticalSignal> detectLibertyShortage(BoardState state, String[] anchors) {
    Set<String> a =
        anchors != null ? new HashSet<>(java.util.Arrays.asList(anchors)) : new HashSet<>();
    a = new HashSet<>();
    for (String s : a) a.add(s.toUpperCase());
    List<BoardGroup> weak = new ArrayList<>();
    for (BoardGroup g : state.groups)
      if (g.liberties.size() <= 2
          && (anchors == null || anchors.length == 0 || groupNearMove(g, a))) weak.add(g);
    if (weak.isEmpty()) return new ArrayList<>();
    TacticalSignal sig = new TacticalSignal();
    sig.type = "liberty-shortage";
    sig.confidence = confidenceFromCount(weak.size());
    for (BoardGroup g : weak.subList(0, Math.min(4, weak.size())))
      sig.evidence.add(
          g.color
              + " group "
              + String.join("/", g.stones.subList(0, Math.min(3, g.stones.size())))
              + " has "
              + g.liberties.size()
              + " liberties: "
              + String.join(",", g.liberties));
    Set<String> rm = new LinkedHashSet<>();
    for (BoardGroup g : weak) {
      rm.addAll(g.stones);
      rm.addAll(g.liberties);
    }
  ...
  public static List<TacticalSignal> detectCutPoints(BoardState state, String[] anchors) {
    ...
    for (BoardGroup g : state.groups) {
      ...
      for (String lib : g.liberties) {
        ...
        for (BoardGroup o : state.groups) {
          ...
          if (o.liberties.contains(lib)) {
            if (o.color.equals(g.color)) friendly.add(o);
            else enemy.add(o);
          }
        }
    ...
    TacticalSignal sig = new TacticalSignal();
    sig.type = "cut-or-connection-point";
    sig.confidence = unique.size() >= 2 ? "high" : "medium";
    for (String p : unique.subList(0, Math.min(6, unique.size())))
      sig.evidence.add(p + " is a shared liberty between friendly connection and enemy pressure.");
  ...
  public static List<TacticalSignal> detectEyeShapeRisk(BoardState state, String[] anchors) {
    ...
    for (BoardGroup g : state.groups)
      if (g.liberties.size() >= 2
          && g.liberties.size() <= 4
          && g.stones.size() >= 3
          && (anchors == null || anchors.length == 0 || groupNearMove(g, a))) cands.add(g);
    if (cands.isEmpty()) return new ArrayList<>();
    TacticalSignal sig = new TacticalSignal();
    sig.type = "eye-shape-risk";
    sig.confidence = cands.size() >= 2 ? "medium" : "low";
    for (BoardGroup g : cands.subList(0, Math.min(3, cands.size())))
      sig.evidence.add(
          g.color
              + " group "
              + String.join("/", g.stones.subList(0, Math.min(4, g.stones.size())))
              + " has compact eye-space candidates "
              + String.join(",", g.liberties)
              + ".");
    Set<String> rm = new LinkedHashSet<>();
  ...
  public static List<TacticalSignal> detectSenteGoteHints(BoardState state, String[] anchors) {
    ...
    for (BoardGroup g : state.groups)
      if (g.liberties.size() == 1
          && (anchors == null || anchors.length == 0 || groupNearMove(g, a))) atari.add(g);
    if (atari.isEmpty()) return new ArrayList<>();
    TacticalSignal sig = new TacticalSignal();
    sig.type = "sente-gote-forcing-move";
    sig.confidence = atari.size() >= 2 ? "high" : "medium";
  ...
  public static List<TacticalSignal> detectTacticalSignals(BoardState state, String[] anchors) {
    ...
    signals.sort(
        (x, y) ->
            (rank.get(y.confidence) - rank.get(x.confidence)) != 0
                ? (rank.get(y.confidence) - rank.get(x.confidence))
                : (y.evidence.size() - x.evidence.size()));
