...
/**
 * 对齐 GoAgent knowledge/training.ts（250 行）： loadKnowledgeTrainingLibrary（解析 training-catalog.json 为
 * KnowledgeTrainingLibrary） + validateKnowledgeTrainingLibrary（duplicate id / title/family /
 * sourceKind / GTP 点 / branches / decisionRules / initialStones / correctMoves / failureMoves /
 * teaching guidance / patternCardId 引用校验）。
 */
public final class TrainingValidator {
  ...
  public static class SourcePolicy {
    ...
    public List<String> allowedKinds =
        new ArrayList<>(Arrays.asList("original", "common-pattern", "licensed-source"));
  }
  ...
  public static class Counts {
    public int josekiLines, lifeDeathProblems, tesujiProblems;
  }
  ...
  public static KnowledgeTrainingLibrary loadKnowledgeTrainingLibrary() {
    ...
    try (InputStream in =
        TrainingValidator.class.getResourceAsStream("/knowledge/training-catalog.json")) {
      if (in == null) {
        cachedRoot = "resources";
        cached = new KnowledgeTrainingLibrary();
        return cached;
      }
      ...
      if (root instanceof Map) {
        ...
        if (m.get("version") instanceof Number)
          lib.version = ((Number) m.get("version")).intValue();
        ...
        if (sp instanceof Map) {
          Map<?, ?> sm = (Map<?, ?>) sp;
          if (sm.get("defaultSourceKind") instanceof String)
            lib.sourcePolicy.defaultSourceKind = (String) sm.get("defaultSourceKind");
          if (sm.get("rule") instanceof String) lib.sourcePolicy.rule = (String) sm.get("rule");
          if (sm.get("allowedKinds") instanceof List) {
            List<String> kinds = new ArrayList<>();
            for (Object k : (List<?>) sm.get("allowedKinds"))
              if (k != null) kinds.add(String.valueOf(k));
  ...
  static void validateSourceKind(String kind, Set<String> allowed, List<String> errors, String id) {
    if (kind == null || !allowed.contains(kind))
      errors.add(id + ": sourceKind must be original, common-pattern, or licensed-source");
  }

  static void validatePointList(List<String> points, List<String> errors, String id, String field) {
    for (String p : points)
      if (!isGtpPoint(p)) errors.add(id + ": invalid GTP point in " + field + ": " + p);
  }

  static void validateStones(
      List<LocalPatternMatcher.BoardSnapshotStone> stones, List<String> errors, String id) {
    if (stones == null || stones.isEmpty()) {
      errors.add(id + ": initialStones must not be empty");
      return;
    }
    for (LocalPatternMatcher.BoardSnapshotStone s : stones) {
      if (!("B".equals(s.color) || "W".equals(s.color)) || !isGtpPoint(s.point))
        errors.add(id + ": invalid initial stone " + s.point + "/" + s.color);
    }
  }

  static void validateMoveExplanations(
      List<LocalShapeGeometryMatcher.TrainingMove> moves,
      List<String> errors,
      String id,
      String field) {
    ...
    for (LocalShapeGeometryMatcher.TrainingMove m : moves)
      if (!isGtpPoint(m.move)) errors.add(id + ": invalid " + field + " move " + m.move);
  }

  static void validatePatternRefs(
      List<String> ids, Set<String> knownPatternIds, List<String> errors, String id) {
    if (knownPatternIds == null) return;
    for (String pid : ids)
      if (!knownPatternIds.contains(pid)) errors.add(id + ": unknown patternCardId " + pid);
  }

  public static KnowledgeTrainingValidationResult validateKnowledgeTrainingLibrary(
      KnowledgeTrainingLibrary lib, Set<String> knownPatternIds) {
    ...
    for (JsonKnowledgeLoader.JosekiLineEntry line : lib.josekiLines) {
      ...
      if (line.title == null
          || line.title.isEmpty()
          || line.family == null
          || line.family.isEmpty()) errors.add(line.id + ": missing title or family");
      ...
      if (line.branches == null || line.branches.isEmpty())
        errors.add(line.id + ": branches must not be empty");
      if (line.decisionRules == null || line.decisionRules.isEmpty())
        errors.add(line.id + ": decisionRules must not be empty");
      validatePatternRefs(
          line.patternCardIds != null ? line.patternCardIds : new ArrayList<>(),
          knownPatternIds,
          errors,
          line.id);
    }
    for (LocalShapeGeometryMatcher.ProblemEntry p : lib.lifeDeathProblems) {
      ...
      ids.add(p.id);
      if (p.title == null || p.title.isEmpty() || p.objective == null || p.objective.isEmpty())
        errors.add(p.id + ": missing title or objective");
      ...
      if (p.teachingRecognition == null
          || p.teachingRecognition.isEmpty()
          || p.teachingFailureExplanation() == null)
        errors.add(p.id + ": missing teaching guidance");
      validatePatternRefs(
          p.patternCardIds != null ? p.patternCardIds : new ArrayList<>(),
          knownPatternIds,
          errors,
          p.id);
    }
